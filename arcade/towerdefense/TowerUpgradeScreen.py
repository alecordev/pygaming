...
def upgrade(screen, tower):
                ...
                try:
                    ...
                    try:
                        next = (
                            c,
                            eval("TowerUpgrades." + c + ".pos"),
                            imgLoad(os.path.join("upgradeicons", c[:2] + ".jpg")),
                        )
                        if c in tower.upgrades:
                            temp = next[2].copy()
                            temp.fill((0, 150, 0))
    ...
    while 1:
        ...
        info = font2.render(
            "Current XP: %d/%d" % (tower.curXP, tower.maxXP), 1, (255, 255, 255)
        )
        ...
        for c, p, i in icons:
            for r in eval("TowerUpgrades." + c + ".req"):
                ...
                pygame.draw.aaline(
                    screen,
                    (255, 255, 255),
                    i.get_rect(topleft=p).move(scrwidoffset, scrheioffset).center,
                    (rp[0] + 10 + scrwidoffset, rp[1] + 10 + scrheioffset),
                )
        for c, p, i in icons:
            screen.blit(i, (p[0] + scrwidoffset, p[1] + scrheioffset))
            if (
                i.get_rect(topleft=p)
                .move(scrwidoffset, scrheioffset)
                .collidepoint(pygame.mouse.get_pos())
            ):
                mouseon = c
                info = font2.render(
                    "%s" % (eval("TowerUpgrades." + c + ".name")), 1, (255, 210, 220)
                )
                infopos = info.get_rect(
        ...
        for event in pygame.event.get():
            if event.type == QUIT:
                ...
                if event.dict["button"] == 1:
                    if mouseon and eval(
                        "TowerUpgrades." + mouseon + ".checkReq(tower)"
                    ):
