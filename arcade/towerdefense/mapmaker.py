...
def saveMenu(screen, background, savefile, movelist, basepos):
    ...
    instructions = font.render(
        "For paths: 1-left,2-top,3-right,4-bottom", 1, (255, 255, 255)
    )
    ...
    for ind in range(4):
        ...
        textpos = text.get_rect(
            left=menurect.left + 10, centery=150 + 50 * len(pathtexts)
        )
    ...
    saverect = savebutton.get_rect(
        centerx=menurect.centerx, bottom=menurect.bottom - 50
    )
    while 1:
...
def runMapMaker():
    ...
    squarelist.append(
        (
            imgLoad(os.path.join("backgroundimgs", "btile.png")),
            pygame.Rect((scrwid + 5, 5 + 25 * len(squarelist)), (20, 20)),
        )
    )
    squarelist.append(
        (
            imgLoad(os.path.join("backgroundimgs", "flag.png")),
            pygame.Rect((scrwid + 5, 5 + 25 * len(squarelist)), (20, 20)),
        )
    )
    ...
    [
        squarelist.append(
            (
                grassbmp.subsurface(pygame.Rect((x * 40, 1 + y * 40), (20, 20))),
                pygame.Rect((scrwid + 5, 5 + 25 * len(squarelist)), (20, 20)),
            )
        )
        for x in range(3)
    ...
    while 1:
                ...
                if event.dict["button"] == 1:
                    ...
                    if event.dict["pos"][0] <= scrwid and event.dict["pos"][0] >= 0:
                        if event.dict["pos"][1] <= scrhei and event.dict["pos"][1] >= 0:
                            background.blit(
                                squarelist[selected - 1][0],
                                (
                                    (event.dict["pos"][0] / 20) * 20,
                                    (event.dict["pos"][1] / 20) * 20,
                                ),
                            )
                            if selected == 1:
        ...
        if not basepos:
            instructions = font.render(
                "1. Click on the B tile to place a base", 1, (255, 255, 255)
            )
