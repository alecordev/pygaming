...
def pickMap(screen, clock):
    maplist = [
        d
        for d in os.listdir(os.path.join("mapfiles"))
        if os.path.isdir(os.path.join("mapfiles", d)) and d[0] != "."
    ]
    ...
    for map in maplist:
        if os.path.isfile(os.path.join("mapfiles", map, "description.txt")):
            text = font.render(
                ...
                % (
                    map,
                    open(os.path.join("mapfiles", map, "description.txt"))
                    .readline()
                    .strip(),
                ),
                1,
...
def pickTower(screen, clock):
    ...
    for tower in localdefs.towerlist:
        tower.rect = tower.image.get_rect(
            centerx=(len(towers) + 1)
            * localdefs.scrwid
            / (len(localdefs.towerlist) + 1.0),
            centery=localdefs.scrhei / 2,
        )
        tower.cost = (
            tower.runes["Alpha"].cost
            * functools.reduce(
                lambda x, y: x * y,
                [
                    tower.runes[k].cost()
                    for k in tower.runes.keys()
                    if k != "Alpha" and tower.runes[k]
                ],
                1,
            )
            if tower.runes["Alpha"]
            else 0
        )
        tower.costText = font.render("%d" % (tower.cost), 1, (0, 0, 0))
        tower.costRect = tower.costText.get_rect(
            centerx=tower.rect.centerx, centery=tower.rect.centery + 25
        )
        towers.append(tower)
    run = 1
    ...
    backtextpos = backtext.get_rect(
        centerx=localdefs.scrwid / 2, centery=localdefs.scrhei - 100
    )
    while run:
        clock.tick(30)
        screen.blit(bg, (0, 0))
...
def editTower(screen, clock, tower):
    ...
    class RP:
        ...
        def testMove(self, point, r):
            if self.outerrect.collidepoint(point):
                if (
                    self.sig in tower.runes.keys()
                    and tower.runes[self.sig]
                    and r[3] == None
                    and r[2].slotIn(self.sig)
                ):
                    replist = None
                    for rlist in runes:
                    ...
                    for rlist in runes:
                        if rlist[3] == None:
                            rlist[1] = rlist[0].get_rect(
                                left=100 + 35 * tempnum, centery=localdefs.scrhei - 150
                            )
                            tempnum += 1
                    return r
    ...
    for k in tower.runes.keys():
        if tower.runes[k]:
            ...
            img.fill(
                (225, 25, 25)
                if tower.runes[k].col == "Red"
                else (25, 225, 25)
                if tower.runes[k].col == "Green"
                else (25, 25, 225)
            )
                ...
                if backtextpos.collidepoint(event.dict["pos"]):
                    ...
                    if mousedown:
                        ...
                        if not towhit and mousedown[3]:
                            ...
                            mousedown[1] = mousedown[0].get_rect(
                                left=100 + 35 * len([r for r in runes if r[3] == None]),
                                centery=localdefs.scrhei - 150,
                            )
                            tower.cost = (
                                tower.runes["Alpha"].cost
                                * functools.reduce(
                            ...
                            tower.costText = font.render(
                                "%d" % (tower.cost), 1, (0, 0, 0)
                            )
                            tower.costRect = tower.costText.get_rect(
                                centerx=tower.rect.centerx,
                                centery=tower.rect.centery + 25,
                            )
                            mousedown = None
            else:
                keyinput = pygame.key.get_pressed()
    ...
    while run:
        ...
        for i, key in enumerate(ImageFiles.keys()):
            ...
            textpos = text.get_rect(
                centerx=(1 + i) * (localdefs.scrwid / (1 + len(ImageFiles))),
                centery=localdefs.scrhei / 2,
            )
            textrectdict[key] = textpos
            imgrect = ImageFiles[key].get_rect(
                centerx=textpos.centerx, bottom=textpos.top - 2
            )
        ...
        for event in pygame.event.get():
            if event.type == QUIT:
                ...
                for key in textrectdict.keys():
                    if textrectdict[key].collidepoint(event.dict["pos"]) or imgrectdict[
                        key
                    ].collidepoint(event.dict["pos"]):
    ...
    for num in range(0, localdefs.player.modDict["towerNum"]):
        ...
        try:
            towerlist[num]["image"] = localdefs.imgLoad(
                os.path.join(
                    "towerimgs",
                    localdefs.player.modDict["tower"][num + 1]["ImageName"] + ".png",
                )
            )
        except:
            towerlist[num]["image"] = localdefs.imgLoad(
                os.path.join("towerimgs", "Fighter.png")
            )
        ...
        towerlist[num]["imagerect"] = towerlist[num]["image"].get_rect(
            ...
            + (num + 1)
            * (localdefs.scrhei - nametextpos.bottom)
            / (localdefs.player.modDict["towerNum"] + 2),
        )
                ...
                if up["rect"].collidepoint(pygame.mouse.get_pos()):
                    ...
                    if pygame.mouse.get_pos()[0] > localdefs.scrwid / 2:
                        screen.blit(
                            text,
                            text.get_rect(
                                right=up["rect"].left - 10, bottom=up["rect"].top
                            ),
                        )
                    else:
                        screen.blit(
                            text,
                            text.get_rect(
                                left=up["rect"].right + 10, bottom=up["rect"].top
                            ),
                        )
            ...
            if event.type == QUIT:
                    ...
                    for tower in towerlist:
                        if tower["imagerect"].collidepoint(event.dict["pos"]):
                            selected["rect"] = selected["img"].get_rect(
                                left=60 + 30 * len(tower["upgrades"]),
                                centery=tower["imagerect"].centery,
                            )
                ...
                if keyinput[K_ESCAPE] or keyinput[K_BACKSPACE]:
                    ...
                    for up in upslist + nondisups:
                        localdefs.player.upgrades.append(
                            up["name"] + str(up["curtower"] + 1)
                        )
                    localdefs.player.save()
                    localdefs.player.__init__()
                    return
...
def main(screen, clock):
    ...
    for num, i in enumerate(["playmap", "edittowers", "options"]):
        ...
        rects[i] = imgs[i].get_rect(
            centerx=localdefs.scrwid / 2, centery=(num + 1) * localdefs.scrhei / 5
        )
    while run:
        clock.tick(30)
        screen.blit(bg, (0, 0))
