...
class Enemy:
    ...
    def attack(self, frametime, screen):
        ...
        if self.currentTargetTimer <= 0:
            ...
            for tower in towerlist:
                if tower.outyet:
                    if (self.rect.centerx - tower.rect.centerx) ** 2 + (
                        self.rect.centery - tower.rect.centery
                    ) ** 2 <= self.hitrangesq:
                        tower.getAttack(
                            (random.randint(1, 2000) * 1.0 / 100.0) + self.attackBonus,
                            self.damage,
                        )
                        self.currentTargetTimer = self.startTargetTimer
                        pygame.draw.line(
                            screen, (255, 0, 0), self.rect.center, tower.rect.center
                        )
                        return
    ...
    def findCloseTower(self):
        for tower in towerlist:
            if (
                tower.outyet
                and tower.hp > 0
                and tower.enemytarget == None
                and (self.rect.centerx - tower.rect.centerx) ** 2
                + (self.rect.centery - tower.rect.centery) ** 2
                <= self.findrangesq
            ):
                return tower
        return None
    ...
    def findTarget(self):
        ...
        if self.towertarget:
            ...
            targetrad = math.atan2(
                self.towertarget.rect.centery - self.rect.centery,
                self.towertarget.rect.centerx - self.rect.centerx,
            )
        else:
            defl = (
                0
                if random.random() >= 0.75
                else random.uniform(-1.0 / 8.0 * math.pi, 1.0 / 8.0 * math.pi)
            )
            targetrad = (
                math.atan2(
                    self.movelist[self.curnode + 1][1] - self.rect.centery,
                    self.movelist[self.curnode + 1][0] - self.rect.centerx,
                )
                + defl
            )
        dis = 40
        self.target = (
            self.rect.centerx + math.cos(targetrad) * dis,
            self.rect.centery + math.sin(targetrad) * dis,
        )
        self.image = pygame.transform.rotozoom(
            EnemyImageArray[self.imgindex][0], targetrad * -180 / math.pi, 1
        )
    ...
    def die(self, tower):
        ...
        player.currentChain = min(
            player.modDict["chainMax"], player.currentChain + 0.01
        )
        Runes.Rune.reward(self, tower)

    def getDamage(self, tower, amt):
        self.health -= (
            max(0, amt - self.damagereduction)
...
class Icon:
    def __init__(self, tower):
        ...
        self.rect = self.img.get_rect(
            left=len(iconlist) * (25) - 20, centery=scrhei - self.img.get_height() + 5
        )
...
class Tower:
    def __init__(self):
        self.tempimage = pygame.transform.smoothscale(
            imgLoad(os.path.join("towerimgs", "Fighter.png")), (squsize, squsize)
        )
    ...
    def writeToolBar(self, screen):
        ...
        for rune in self.runes.keys():
            if self.runes[rune]:
                ...
                screen.blit(t, tp)
                t = font.render(
                    "%s(%d)" % (self.runes[rune].name, self.runes[rune].cost),
                    1,
                    (0, 0, 0),
                )
                ...
                bigrect = t.get_rect(
                    left=upgradeButtonRect.left, bottom=bigrect.top - 23
                )
        ...
        try:
            for towername in os.listdir(os.path.join(player.name)):
                if "tower" in towername:
                    ...
                    nt.tempimage = pygame.transform.smoothscale(
                        imgLoad(os.path.join("towerimgs", "Fighter.png")),
                        (squsize, squsize),
                    )
    ...
    def saveTowers(self):
        ...
        for ind, tower in enumerate(towerlist[:]):
            ...
            pickle.dump(
                tower, open(os.path.join(player.name, "tower" + str(ind) + ".obj"), "w")
            )
    ...
    def takeTurn(self, frametime, screen):
        if self.hp > 0:
            self.hp = min(
                self.maxhp, self.hp + frametime / 5.0 * self.healthRegeneration
            )
            if self.runes["Alpha"]:
