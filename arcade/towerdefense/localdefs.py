...
class Map:
    ...
    def getmovelist(self):
        ...
        self.basepoint = (
            int(int(line[0]) * 20.0 / squsize),
            int(int(line[1]) * 20.0 / squsize),
        )
        ...
        for line in f.readlines():
            ...
            if (
                int(line[0]) < 0
                or int(line[1]) < 0
                or int(line[0]) > (scrwid)
                or int(line[1]) > (scrhei / squsize)
            ):
                movelists.append(list())
                movelistnum += 1
            movelists[movelistnum].append(
                (int(int(line[0]) * 20.0 / squsize), int(int(line[1]) * 20.0 / squsize))
            )
        ...
        for movelist in movelists:
            ...
            pointmovelist = list(
                [
                    (
                        point[0] * squsize + int(squsize / 2.0),
                        point[1] * squsize + int(squsize / 2.0),
                    )
            ...
            pathrectlist = list(
                [
                    pygame.Rect(
                        pointmovelist[ind],
                        (
                            pointmovelist[ind + 1][0] - pointmovelist[ind][0],
                            pointmovelist[ind + 1][1] - pointmovelist[ind][1],
                        ),
                    )
                    for ind in range(len(pointmovelist) - 2)
                ]
            )
    ...
    def backgroundGen(self, bgsize):
        ...
        dp = pygame.transform.smoothscale(
            imgLoad(os.path.join("backgroundimgs", "roadsquare.jpg")),
            (squsize, squsize),
        )

        background = imgLoad(
            os.path.join("mapfiles", str(self.current), "background.jpg")
        )
        for pathnum in range(len(self.pointmovelists)):
            ...
            for rec in [
                pygame.Rect(x, y, squsize, squsize)
                for x in range(0, scrwid, squsize)
                for y in range(0, scrhei, squsize)
            ]:
                if any([rec.collidepoint(point) for point in pointmovelist]):
                    background.blit(dp, rec)
                elif (
                    len(list([rect for rect in pathrectlist if rect.colliderect(rec)]))
                    == 2
                ):
                    background.blit(dp, rec)
                else:
        ...
        self.baserect = self.baseimg.get_rect(
            center=(
                self.basepoint[0] * squsize + (0.5 * squsize),
                self.basepoint[1] * squsize + (0.5 * squsize),
            )
        )
...
class Player:
    ...
    def save(self):
        ...
        pickle.dump(
            self.spareRunes, open(os.path.join(player.name, "skilllist.obj"), "w")
        )
        del self.spareRunes[:]
...
def distance(first, second):
    return math.sqrt(
        (second.centerx - first.centerx) ** 2 + (second.centery - first.centery) ** 2
    )
...
def genEnemyImageArray():
    for type in [
        ...
        ia = list()
        try:
            enemyimage = imgLoad(os.path.join("enemyimgs", type + ".png"))
        except:
