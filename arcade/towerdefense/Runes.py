...
class SingleAttack(Rune):
    ...
    def speed(self):
        return (
            self.maxspeed
            * (1 - self.tower.speedIncrease)
            * functools.reduce(
                lambda x, y: x * y,
                [
                    self.tower.runes[k].speed()
                    for k in self.tower.runes.keys()
                    if k != "Alpha" and self.tower.runes[k]
                ],
                1,
            )
        )
    ...
    def damageModifier(self):
        return (
            (1 + self.tower.phyDamIncrease)
            * self.damageBonus
            * (1 + self.tower.damIncrease)
            + self.tower.phyDamBonus
            + self.tower.damBonus
        )
    ...
    def run(self, screen, time):
        ...
        if self.tempspeed <= 0:
            if (
                self.tower.enemytarget
                and self.tower.enemytarget in localdefs.enemylist
                and (self.tower.rect.centerx - self.tower.enemytarget.rect.centerx) ** 2
                + (self.tower.rect.centery - self.tower.enemytarget.rect.centery) ** 2
                <= self.tower.rangesq()
            ):
                hitrect = self.tower.enemytarget.rect
                self.tower.enemytarget.getDamage(
                    self.tower, random.uniform(self.minDamage(), self.maxDamage())
                )
                enemypoint = hitrect.center
            else:
                for enemy in sorted(
                    localdefs.enemylist, key=(lambda x: x.distance), reverse=True
                ):
                    if (
                        not enemypoint
                        and (self.tower.rect.centerx - enemy.rect.centerx) ** 2
                        + (self.tower.rect.centery - enemy.rect.centery) ** 2
                        <= self.tower.rangesq()
                    ):
                        enemy.getDamage(
                            self.tower,
                            random.uniform(self.minDamage(), self.maxDamage()),
                        )
            ...
            if enemypoint:
                ...
                pygame.draw.line(
                    screen, (255, 255, 255), self.tower.rect.center, enemypoint
                )
