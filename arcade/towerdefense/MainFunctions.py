...
def workEnemies(screen, frametime):
    ...
    for enemy in enemylist:
        ...
        pygame.draw.line(
            screen,
            (0, 0, 0),
            (enemy.rect.left, enemy.rect.top - 2),
            (enemy.rect.right, enemy.rect.top - 2),
            3,
        )
        if enemy.poisontimer:
            pygame.draw.line(
                ...
                (
                    enemy.rect.left
                    + (enemy.health * 1.0 / enemy.starthealth * 1.0) * enemy.rect.width,
                    enemy.rect.top - 2,
                ),
                3,
            )
        ...
        if enemy.rect.collidepoint(pygame.mouse.get_pos()) and not infobox:
            ...
            if enemy.attackBonus and enemy.damage:
                ...
                infoboxback.blit(t, tp)
            t = font.render(
                "%dcr; %0.1fxp" % (enemy.cost, enemy.xp), 1, (175, 75, 175, 255)
            )
            tp = t.get_rect(centerx=infoboxback.get_width() / 2, top=tp.bottom + 4)
            infoboxback.blit(t, tp)
            if enemy.leveladj:
                ...
                infoboxback.blit(t, tp)
            ibrect = (
                infoboxback.get_rect(topleft=pygame.mouse.get_pos())
                if pygame.mouse.get_pos()[1] + infoboxback.get_height() < scrhei
                else infoboxback.get_rect(bottomleft=pygame.mouse.get_pos())
            )
    ...
    for struct in towerlist:
        if struct.outyet:
            ...
            pygame.draw.line(
                screen,
                (0, 0, 0),
                (struct.rect.left, struct.rect.top - 2),
                (struct.rect.right, struct.rect.top - 2),
                3,
            )
            if struct.hp > 0:
                pygame.draw.line(
                    ...
                    (
                        struct.rect.left
                        + (struct.hp * 1.0 / struct.maxhp * 1.0) * struct.rect.width,
                        struct.rect.top - 2,
                    ),
                    3,
                )
                ...
                pygame.draw.line(
                    ...
                    (
                        struct.rect.left
                        + ((10 + struct.hp) * 1.0 / (10.0)) * struct.rect.width,
                        struct.rect.top - 2,
                    ),
                    3,
                )
...
def selectedIcon(screen, selected):
    ...
    screen.blit(
        selected.img,
        selected.img.get_rect(
            center=(mouseat[0] + squsize / 2.0, mouseat[1] + squsize / 2.0)
        ),
    )
        ...
        if icon.rect.collidepoint(mousepos):
            ...
            if icon is mapvar.iconhold[0]:
                ...
                if mapvar.iconhold[1] > 3:
                    text = font.render(
                        "Speed %0.1f" % (icon.tower.speed()), 1, (0, 0, 0)
                    )
                    textpos = text.get_rect(
                        left=icon.rect.left, bottom=icon.rect.top - 2
                    )
                    screen.blit(text, textpos)
                    text = font.render(
                        "Damage %0.1f-%0.1f"
                        % (icon.tower.minDamage(), icon.tower.maxDamage()),
                        1,
                        (0, 0, 0),
                    )
                    ...
                    screen.blit(text, textpos)
                    text = font.render(
                        "Range %0.1f" % (icon.tower.range()), 1, (0, 0, 0)
                    )
                    textpos = text.get_rect(left=icon.rect.left, bottom=textpos.top - 2)
                    screen.blit(text, textpos)
                    text = font.render(
                        "Armor Class %0.1f" % (icon.tower.ac), 1, (0, 0, 0)
                    )
                    textpos = text.get_rect(left=icon.rect.left, bottom=textpos.top - 2)
                    screen.blit(text, textpos)
                    text = font.render(
                        "Max HP %0.1f" % (icon.tower.maxhp), 1, (0, 0, 0)
                    )
                    textpos = text.get_rect(left=icon.rect.left, bottom=textpos.top - 2)
                    screen.blit(text, textpos)
    ...
    for event in pygame.event.get():
        if event.type == QUIT:
            ...
            if keyinput[K_ESCAPE]:
                ...
                event.type == KEYDOWN
                and event.dict["key"] == K_n
                and not didrollthisturn
            ):
