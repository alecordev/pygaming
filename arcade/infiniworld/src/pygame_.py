...
class FPSSprite(pygame.sprite.Sprite):
    ...
    def update(self):
        ...
        self._old_fps = fps
        text = "%i FPS" % fps
        self.image = self._font.render(text, True, (255, 255, 255), (32, 0, 32))
...
def makeTileImages(elevation):
    ...
    def draw3dBorder(image, color, thickness):
        ...
        for i in range(thickness):
            ...
            pygame.draw.line(image, color_dark, (max_x - i, max_y - i), (i, max_y - i))
            pygame.draw.line(
                image, color_dark, (max_x - i, max_y - i), (max_x - i, i + 1)
            )
    ...
    colors = {
        models.tile.NATURE_DEEPWATER: (92, 138, 202),
    ...
    for nature, color in colors.items():
...
class PygameController(evtman.SingleListener):
    ...
    def _scanKeyboard(self):
        ...
        new_pressed = set((key for key in range(len(pressed)) if pressed[key]))
    ...
    def _keyDown(self, pygame_event):
        ...
        if char:
            self.post(controllers.keyboard.LetterTypedEvent(str))
    ...
    def onProcessInputsEvent(self, x, unused):
...
class PygameView(evtman.SingleListener):
    ...
    def takeScreenShot(self):
        ...
        suffix = 0
        while True:
            file_name = "%i-%02i-%02i_%02i-%02i-%02i-%i.png" % (
                now.tm_year,
                now.tm_mon,
                now.tm_mday,
                now.tm_hour,
    ...
    def onRenderFrameEvent(self, listener=None, event=None):
...
class CoordinatesConverter(object):
    ...
    def worldToPix(self, pos_vector):
        ...
        # says otherwise.
        return (
            self._ref_pix_x + int(round(offset.x)),
...
class EntityView(evtman.SingleListener):
    ...
    area_view = property(
        _getAreaView, _setAreaView, None, "AreaView displaying this EntityView."
    )
    ...
    def render(self):
        ...
        pygame.draw.circle(self.sprite.image, (255, 255, 255, 255), (16, 16), 16)
...
class AreaView(evtman.SingleListener):
    ...
    def renderTiles(self):
        ...
        x_max, y_min = self._coord_conv.pixToWorld(size[0], size[1])
    ...
    def onEntityLeftAreaEvent(self, x=None, event=None):
        """An entity left an area."""
        if event.area_id == self.area_id:
            self.destroyEntityView(event.entity_id)

    def onAreaContentEvent(self, x=None, event=None):
    ...
    def onViewAreaEvent(self, x=None, event=None):
        """We are looking at a new area."""
        self.setAreaId(event.area_id)

    def onControlEntityEvent(self, x=None, event=None):
