...
def chunkCoordsAround(x, y, radius, scale):
    ...
    chunk_x_min = int(-((0.5 - x_min) // 1))  # These are integers:
    chunk_x_max = int((0.5 + x_max) // 1)  # Coordinates of the chunks to
    chunk_y_min = int(-((0.5 - y_min) // 1))  # consider for collisions.
    chunk_y_max = int((0.5 + y_max) // 1)
...
class EntityMap(object):
    ...
    def getNear(self, pos, radius):
        ...
        """
        x, y = pos
        x_min, x_max, y_min, y_max = chunkCoordsAround(x, y, radius, self.scale)
        result = set()
