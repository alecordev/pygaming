...
from .errors import AlreadyInAreaError, NotInAreaError
...
def tileCoordAt(pos):
    ...
    """
    tile_x = int((0.5 + pos.x) // 1)
    tile_y = int((0.5 + pos.y) // 1)
...
def tileCoordsAround(x, y, radius):
    ...
    y_min = y - radius
    y_max = y + radius
    tile_x_min = int(-((0.5 - x_min) // 1))  # These are integers:
    tile_x_max = int((0.5 + x_max) // 1)  # Coordinates of the tiles to
    tile_y_min = int(-((0.5 - y_min) // 1))  # consider for collisions.
    tile_y_max = int((0.5 + y_max) // 1)
...
class AreaModel(SingleListener):
    ...
    def pruneTiles(self, entity):
        ...
        """
        x, y = entity.body.pos
        x_min, x_max, y_min, y_max = tileCoordsAround(x, y, entity.body.radius)
        coords = set()
    ...
    def detectCollisionsWithTiles(self, collider):
        ...
        for coord in coords:
            ...
            tile_body = physics.RectangularBody(
                float("inf"), geometry.Vector(coord), True, material, 1.0, 1.0
            )
    ...
    def detectCollisionsWithEntities(self, collider):
        """Return a set of Collision objects."""
        collisions = set()
        entities = self.entity_map.getNear(
            collider.body.pos, collider.body.radius + self._biggest_entity_radius
        )
    ...
    def processCollisions(self, entity):
        ...
        if collisions:
            ...
            collisions = sorted(collisions, key=attrgetter("distance"), reverse=True)
            while collisions:
    ...
    # --------------------------------  Events.  -------------------------------
    def onAreaContentRequest(self, x=None, event=None):
        ...
        if event.area_id == self.area_id:
            entities = [entity.makeSummary() for entity in self.entities.values()]
            tilemap = self.tile_map.makeSummary()
            self.post(events.AreaContentEvent(self.area_id, entities, tilemap))

    def onRunPhysicsEvent(self, x=None, event=None):
