...
class PlayerController(evtman.SingleListener):
    ...
    def onStartMovingEastCommand(self, *args):
        """Players wants to move East."""
        if not self.going_east:
            self.going_east = 1
            self.moveEntity()

    def onStartMovingNorthCommand(self, *args):
        """Players wants to move North."""
        if not self.going_north:
            self.going_north = 1
            self.moveEntity()

    def onStartMovingWestCommand(self, *args):
        """Players wants to move West."""
        if not self.going_west:
            self.going_west = 1
            self.moveEntity()

    def onStartMovingSouthCommand(self, *args):
        """Players wants to move South."""
        if not self.going_south:
            self.going_south = 1
            self.moveEntity()

    def onStopMovingEastCommand(self, *args):
        """Players stops moving East."""
        if self.going_east:
            self.going_east = 0
            self.moveEntity()

    def onStopMovingNorthCommand(self, *args):
        """Players stops moving North."""
        if self.going_north:
            self.going_north = 0
            self.moveEntity()

    def onStopMovingWestCommand(self, *args):
        """Players stops moving West."""
        if self.going_west:
            self.going_west = 0
            self.moveEntity()

    def onStopMovingSouthCommand(self, *args):
        """Players stops moving South."""
        if self.going_south:
            self.going_south = 0
            self.moveEntity()

    def onFireCommand(self, *args):
