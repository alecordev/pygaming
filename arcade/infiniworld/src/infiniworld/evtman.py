...
class Event(object):
    ...
    def __init__(self, *args):
        ...
        if len(self.attributes) != len(args):
            msg = (
                "Incorrect number of arguments for %s: %i needed, "
...
class Listener(object):
    ...
    def isHandler(self, name):
        ...
        """
        return (
            name.startswith(self.HANDLER_PREFIX)
    ...
    def onEvent(self, unused):
        """Dummy manager.  Keep it here.

        This ensures that listeners have at least one handler.  If they don't,
        then a NotRegisteredError is raised when they are unregistered.  Now,
        I could ask the event manager not to complain, but that would silence
...
class EventManager(object):
    ...
    def register(self, listener):
        ...
        # Event class name.
        for event_name, handler in handlers_for_listener.items():
    ...
    def pump(self):
        ...
        for event in self._event_queue:
            ...
            if handlers:
                for listener, handler in list(handlers.items()):
