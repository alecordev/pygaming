...
class Game(evtman.SingleListener):
    ...
    def __init__(self, event_manager):
        ...
        #
        with pygame_.Pygame():
            self._pygame_view = pygame_.PygameView(
                event_manager, u"Apocalypse Bunny", (800, 480)
            )
            assets.load()
    ...
    def onStartGameCommand(self, unused, x=None):
    ...
    def onGameOverEvent(self, *args):
        self._keyboard_controller.unregister()
        self._game_over_view = views.GameOverScreenView(self._event_manager, self._time)
        self._pygame_view.addView(self._game_over_view)
        self._keyboard_controller = GameOverScreenKeyboardController(
            self._event_manager
        )

    def onPhysicsPausedEvent(self, x=None, event=None):
