...
def GenerateInterestingTileMap(size, obstacle_density):
    ...
    natures = list(infiniworld.models.tile.NATURES_FROM_ID.keys())
...
def GenerateWorld(event_manager, area_size):
    ...
    tile_map = GenerateInterestingTileMap(area_size, 0.2)
    coords = set(
        [coord for coord, tile_ in tile_map.tiles.items() if tile_.height == 0]
    )
