...
class CreatureModel(EntityModel):
    ...
    def onHealthRequest(self, x=None, event=None):
...
class BunnyModel(CreatureModel):
    ...
    def onAttackRequest(self, x=None, event=None):
...
class ZombieFoxModel(CreatureModel):
    ...
    def randomWalk(self):
        ...
        self._change_direction_cooldown *= 0.8 + 0.4 * random.random()
        angle = random.random() * 2 * math.pi
        self._walk_force.vector = Vector.fromDirection(angle, self.WALK_STRENGTH)
    ...
    def runAI(self, timestep):
        ...
        entities = self.area.entity_map.getNear(self.body.pos, self.PERCEPTION_RADIUS)
        entities = [
            entity for entity in entities if entity.NAME == "Bunny" and entity.exists
        ]
        if entities:
            # Sort by distance.
            distances = [entity.body.pos.dist(self.body.pos) for entity in entities]
            entities = list(zip(entities, distances))
...
class SpawnerModel(SingleListener):
    ...
    def onRunPhysicsEvent(self, x=None, event=None):
        ...
        for unused in range(int(how_many)):
            self.spawn()
