...
class TCPServer:
    ...
    def connect(self, host, port):
        ...
        try:
            ...
            raise ServerError(
                "Only one instance of the server on port "
                + str(self.port)
                + " may run at one time!"
            )
        ...
        while self.looping:
            ...
            for sock in input_ready:
                if sock == self.unconnected_socket:
                    ...
                    self.client_connect_func(
                        connected_socket, self.host, self.port, address
                    )
                else:
                    try:
...
class TCPClient:
    ...
    def connect(self, host, port):
        ...
        try:
            ...
            self.socket.close()
            raise SocketError(
                "The connection could not be opened.  It must be created first with a server object."
            )
