...
class ServerHandler(TCPServer):
    ...
    def client_connect_func(self, sock, host, port, address):
        print(
            "A client, (ip: %s, code: %s) connected on port %s!"
            % (address[0], address[1], port,)
        )

    def client_disconnect_func(self, sock, host, port, address):
        print(
            "A client, (ip: %s, code: %s) disconnected from port %s!"
            % (address[0], address[1], port,)
        )
    ...
    def handle_data(self, data):
        ...
        if action == "add message":
            ...
            self.s_id += 1
            self.add_message(
                str(self.s_id) + "|MOVE|" + str(data[3]) + "|" + str(data[2])
            )
        elif action == "bomb":
            self.s_id += 1
            self.add_message(
                str(self.s_id) + "|BOMB|" + str(data[3]) + "|" + str(data[2])
            )
