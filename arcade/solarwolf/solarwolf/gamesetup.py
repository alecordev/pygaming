...
def load_game_resources():
    ...
    t = txt.Font("sans", 12).text(
        (180, 220, 180),
        "(You can Pause the game with the PAUSE or P buttons)",
        (400, 590),
    )
    images.append(t)
...
class GameSetup:
    ...
    def moveto(self, pos):
        if self.shipdir == SHIPUP:
            ...
            self.shippos = (
                pos[0] - (self.images[0][1].width // 2),
                pos[1] - self.images[0][1].height,
            )
        elif self.shipdir == SHIPRIGHT:
            self.shippos = (
                pos[0] - self.images[0][1].width,
                pos[1] - (self.images[0][1].height // 2),
            )
        else:
    ...
    def add(self, i):
        if not i.release:
            if i.all:
                snd.play("select_choose")
                self.clearactionlist()
                self.display[input.actions_order[self.currentaction]].append(
                    (i.type, i.normalized)
                )
    ...
    def selectdelete(self):
        ...
        mutable = len(
            list(
                filter(ignoreall, self.display[input.actions_order[self.currentaction]])
            )
        )
        if mutable > 1:
            snd.play("select_choose")
    ...
    def selectadd(self):
        if len(self.display[input.actions_order[self.currentaction]]) <= 12:
            ...
            self.currentcontrol = len(
                self.display[input.actions_order[self.currentaction]]
            )
            self.moveto(self.targetcontrol())
        else:
            snd.play("incorrect")
    ...
    def selectall(self):
        ...
        input.translations[NOEVENT][JOYBUTTONDOWN] = input.actions_order[
            self.currentaction
        ]
        ...
        if self.inputstate == BUTTONS:
            ...
            if i.translated in (input.DOWN, input.UP, input.LEFT, input.RIGHT):
                if i.translated == input.DOWN or i.translated == input.UP:
                    ...
                    if currentcontrol < len(
                        self.display[input.actions_order[self.currentaction]]
                    ):
                        self.currentcontrol = currentcontrol
                elif i.translated == input.LEFT:
                    self.currentcontrol = (self.currentcontrol - 1) % 6 + 6 * (
                        self.currentcontrol // 6
                    )
                    if self.currentcontrol >= len(
                        self.display[input.actions_order[self.currentaction]]
                    ):
                        self.currentcontrol = (
                            len(self.display[input.actions_order[self.currentaction]])
                            - 1
                        )
                elif i.translated == input.RIGHT:
                    self.currentcontrol = (self.currentcontrol + 1) % 6 + 6 * (
    ...
    def targetbutton(self):
        ...
        y = (
            self.actionlist[self.currentaction][1].top
            + self.buttonlist[self.currentbutton][1].centery
        )
        return (x, y)
    ...
    def targetcontrol(self):
        ...
        y = (
            self.actionlist[self.currentaction][1].top
            + self.controlrectlist[self.currentcontrol].centery
        )
        return (x, y)
    ...
    def buildstatus(self):
        if self.status != "...":
            ...
            self.statusimage = textfont.text(
                (255, 250, 160), statustext, (gfx.rect.centerx, 560)
            )
        else:
            self.statusimage = None
