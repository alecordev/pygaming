...
class GameMenu:
    def __init__(self, prevhandler):
        ...
        self.version = fnt.text(
            (100, 200, 120), "SolarWolf Version " + game.version, (10, 580), "topleft"
        )
    ...
    def renderhall(self):
        ...
        for p in winners:
            if not firstone:
                pygame.draw.line(
                    img, (50, 50, 100), (left - 80, 20), (left - 80, 60), 1
                )
            ...
            if p.cheater:
                ...
                if p.skips:
                    img.blit(
                        *smallfont.text(
                            (160, 160, 160),
                            "%d ships, %d skips" % (p.lives, p.skips),
                            (left, 58),
                        )
                    )
                else:
                    img.blit(
                        *smallfont.text(
                            (160, 160, 160), "%d ships" % p.lives, (left, 58)
                        )
                    )
            left += 160
            firstone = 0
