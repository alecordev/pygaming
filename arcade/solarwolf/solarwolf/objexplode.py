...
class Explode:
    def __init__(self, pos, move=(0, 0)):
        self.time = 0.0
        self.move = [float(move[0]) * 0.9, float(move[1]) * 0.9]
...
class Debris:
    def __init__(self, part, pos, move=(0, 0)):
        ...
        self.move[0] += (random.random() - 0.5) * 2.0
        self.move[1] += (random.random() - 0.5) * 2.0
        self.pos = [
            float(pos[0]) + self.move[0] * 1.5,
            float(pos[1]) + self.move[1] * 1.5,
        ]
        ...
        self.dead = 0
        self.lastrect = None
        self.speed = random.random() * 0.15 + 0.15
...
def superexplode(pos, move):
    ...
    for x in range(4):
        newpos = list(pos)
        newpos[0] += (random.random() - 0.5) * 30
        newpos[1] += (random.random() - 0.5) * 30
