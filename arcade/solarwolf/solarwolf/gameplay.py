...
class GamePlay:
    ...
    def normal_tick(self):
        ...
        if (
            self.player.turbo
            and (self.player.move[0] or self.player.move[1])
    ...
    def playerdie_start(self):
        ...
        self.staticobjs.extend(
            objexplode.superexplode(self.player.rect.center, self.player.move)
        )
    ...
    def levelstart_end(self):
        ...
        pct = 1.0 - (pct * pct)
        pct = pct * 0.92
        game.guard_fire = 0.01 + pct * game.fire_factor
        if len(self.boxobjs):
    ...
    def gamestart_tick(self):
        ...
        if not self.donehud:
            ...
            if (
                ...
                and self.levelnum == self.startlevel - 1
            ):
                self.changestate("levelstart")
    ...
    def gameover_tick(self):
        if game.timeleft:
            ...
            if not self.ticks and not self.textobjs:
                for x in self.guardobjs:
                    x.dead = 1
                for x in self.boxobjs:
                    x.dead = 1
