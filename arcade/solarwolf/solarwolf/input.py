...
hat_direction_text = {HATUP: "Up", HATLEFT: "Left", HATRIGHT: "Right", HATDOWN: "Down"}
...
actions_order = [UP, DOWN, LEFT, RIGHT, PRESS, ABORT]
...
def postactive():
    ...
    for key in range(len(keystate)):
        if keystate[key]:
            ...
            pygame.event.post(
                pygame.event.Event(KEYDOWN, {"key": key, "mod": pygame.key.get_mods()})
            )
    if joystick:
        for button in range(joystick.get_numbuttons()):
            if joystick.get_button(button):
                pygame.event.post(
                    pygame.event.Event(
                        JOYBUTTONDOWN, {"joy": joystick.get_id(), "button": button}
                    )
                )
        for hat in range(joystick.get_numhats()):
            ...
            if 0 != value[0] or 0 != value[1]:
                pygame.event.post(
                    pygame.event.Event(
                        JOYHATMOTION,
                        {"joy": joystick.get_id(), "hat": hat, "value": value},
                    )
                )
