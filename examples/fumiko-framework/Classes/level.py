...
from .common import Size, Position, read_dict_from_file, debug
            ...
            for object_name, object_settings in level.settings["gobjects"].items():
                gobject_from_level_dict = game.add(
                    gameobject.GameObject(
                        object_name,
                        Position(
                            object_settings["position"][0],
                            object_settings["position"][1],
                        ),
                        Size(object_settings["size"][0], object_settings["size"][1]),
                    )
                )
    ...
    def populate(self):
        ...
        for i in range(0, len(self.heightmap)):
            self.populated.append(
                Terrain(i, int(self.heightmap[i]), int(self.terrainmap[i]))
            )

    def slice_chipset(self):
        self.terrains = []
        chipset = get_image(self.tileset)
        chipset_rect = chipset.get_rect()
        number_of_possible_slices = int(chipset_rect.width / self.tile_width)

        for i in range(0, number_of_possible_slices):
            subsurface = chipset.subsurface(
                pygame.Rect(
            ...
            subsurface = pygame.transform.scale(
                subsurface, (tile_rect.width * scale, tile_rect.height * scale)
            )
            self.terrains.append(subsurface)
        debug(
            self.terrains,
            "Created " + str(number_of_possible_slices) + " Tiles from " + self.tileset,
        )
