...
class PhysicsController2D:
    ...
    def check_for_collision(self, point=None):
        for controller in game.physics_controllers:
            if point is None:
                if self is not controller and self.collider.colliderect(
                    controller.collider
                ):
                    return True
            else:
    ...
    def is_grounded(self):
        groundingPoints = []
        groundingStart = (
            self.collider.midbottom[0] - self.groundingWidth / 2,
            self.collider.midbottom[1],
        )
        for i in range(0, self.groundingDensity):
            groundingPoints.append(
                (
                    groundingStart[0]
                    + (self.groundingWidth / self.groundingDensity) * i,
                    groundingStart[1],
                )
            )
