...
class PygcurseSurface(object):
    ...
    def __init__(
        ...
        windowsurface=None,
        inputcursorblinking=True,
        inputcursorblinkspeed=1000,
    ):
        """
        ...
        self._cellwidth, self._cellheight = calcfontsize(
            self._font
        ...
        if windowsurface == _NEW_WINDOW:
            self._windowsurface = pygame.display.set_mode(
                (self._cellwidth * width, self._cellheight * height)
            )
            self._managesdisplay = True
        elif windowsurface == FULLSCREEN:
            self._windowsurface = pygame.display.set_mode(
                (self._cellwidth * width, self._cellheight * height), pygame.FULLSCREEN
            )
            self._managesdisplay = True
        else:
        ...
        self._surfaceobj = (
            self._surfaceobj.convert_alpha()
        ...
        while True:  # the event loop
            ...
            if callbackfn:
                try:
                    ...
                    if (
                        e.args[0]
                        == "callbackfn() takes 0 positional arguments but 1 was given"
                    ):
                        callbackfn()
                    else:
                        # if not, reraise it.
...
'''
        ...
        for x in range(self._width):
            for y in range(self._height):
                ...
                if self._screendirty[x][y]:  # draw to surfaceobj all the dirty cells.
                    ...
                    charsurf = self._font.render(
                        self._screenchar[x][y], 1, cellfgcolor, cellbgcolor
                    )
                    ...
                    charrect.bottom = self._cellheight * (
                        y + 1
                    )  # TODO - not correct, this would put stuff like g, p, q higher than normal.
    ...
    def _drawinputcursor(self):
        ...
        if (
            self._inputcursormode is not None
            and self._inputcursorx is not None
            and self._inputcursory is not None
        ):
            ...
            if (
                not self.inputcursorblinking
                or ((microseconds // 1000) // self.inputcursorblinkspeed) % 2
            ):
                cellfgcolor, cellbgcolor = self.getdisplayedcolors(x, y)
                ...
                if self._inputcursormode == "underline":
                    ...
                    pygame.draw.rect(
                        ...
                        (
                            self._cellwidth * x + 2,
                            self._cellheight * (y + 1) - int(self._cellheight / 2.5),
                            self._cellwidth - 4,
                            int(self._cellheight / 2.5),
                        ),
                    )
                    ...
                    charsurf = self._font.render(
                        self._screenchar[x][y], 1, cellbgcolor, cellfgcolor
                    )
                    charrect = charsurf.get_rect()
    ...
    def getdisplayedcolors(self, x, y):
        ...
        fgcolor = (
            (self._screenfgcolor[x][y] is None)
            and (DEFAULTFGCOLOR)
            or (self._screenfgcolor[x][y])
        )
        bgcolor = (
            (self._screenbgcolor[x][y] is None)
            and (DEFAULTBGCOLOR)
            or (self._screenbgcolor[x][y])
        )

        # NOTE - The ternary trick does work here, because the case where the wrong value of the two is used, both values are the same.
        rdelta = (
            (self._screenRdelta[x][y] is not None) and (self._screenRdelta[x][y]) or (0)
        )
        gdelta = (
            (self._screenGdelta[x][y] is not None) and (self._screenGdelta[x][y]) or (0)
        )
        bdelta = (
            (self._screenBdelta[x][y] is not None) and (self._screenBdelta[x][y]) or (0)
        )
        ...
        if rdelta or gdelta or bdelta:
            ...
            r, g, b, a = (
                self._screenbgcolor[x][y].r,
                self._screenbgcolor[x][y].g,
                self._screenbgcolor[x][y].b,
    ...
    def _repaintcell(self, x, y):
        ...
        cellfgcolor, cellbgcolor = self.getdisplayedcolors(x, y)
        cellrect = pygame.Rect(
            self._cellwidth * x, self._cellheight * y, self._cellwidth, self._cellheight
        )
        self._surfaceobj.fill(cellbgcolor, cellrect)
        charsurf = self._font.render(
            self._screenchar[x][y], 1, cellfgcolor, cellbgcolor
    ...
    def _debugfgFn(self, x, y):
        r, g, b = (
            self._screenfgcolor[x][y].r,
            self._screenfgcolor[x][y].g,
            self._screenfgcolor[x][y].b,
        )
    ...
    def getcoordinatesatpixel(self, pixelx, pixely=None, onscreen=True):
        ...
        if (
            onscreen
            and (pixelx < 0 or pixelx >= self._width * self._cellwidth)
            or (pixely < 0 or pixely >= self._height * self._cellheight)
        ):
            return (None, None)
    ...
    def resize(self, newwidth=None, newheight=None, fgcolor=None, bgcolor=None):
        ...
        if self._managesdisplay:
            ...
            self._windowsurface = pygame.display.set_mode(
                (self._pixelwidth, self._pixelheight)
            )
            self.update()
    ...
    def reversecolors(self, region=None):
        ...
        for ix in range(regionx, regionx + regionwidth):
            for iy in range(regiony, regiony + regionheight):
                self._screenfgcolor[ix][iy], self._screenbgcolor[ix][iy] = (
                    self._screenbgcolor[ix][iy],
    ...
    def paste(
        ...
        if self == dstsurf and regionsoverlap(
        ...
        for ix in range(srcx, srcx + srcwidth):
            for iy in range(srcy, srcy + srcheight):
                ...
                if (
                    not dstsurf.isonscreen(finx, finy)
                    or ix - srcx >= dstwidth
                    or iy - srcy >= dstheight
                ):
                    continue
    ...
    def pastechars(self, srcregion=None, dstsurf=None, dstregion=None):
        return self.paste(
            srcregion, dstsurf, dstregion, True, False, False, False, False, False
        )
    ...
    def addshadow(
        self, amount=51, region=None, offset=None, direction=None, xoffset=1, yoffset=1
    ):
        """
        ...
        if yoffset < 0 and (-width < xoffset < width):
            self.darken(
                amount,
                (
                    x + getwithinrange(xoffset, 0, width),
                    y + yoffset,
                    width - abs(xoffset),
        ...
        if xoffset > 0 and yoffset < 0:
            self.darken(
                amount,
                (
                    x + getwithinrange(xoffset, width, xoffset),
                    y + yoffset,
                    min(abs(xoffset), width),
                    min(abs(yoffset), height),
                ),
    ...
    def tint(self, r=0, g=0, b=0, region=None):
        ...
        for ix in range(x, x + width):
            for iy in range(y, y + height):
                self._screenRdelta[ix][iy] = getwithinrange(
                    r + self._screenRdelta[ix][iy], min=-255
                )
                self._screenGdelta[ix][iy] = getwithinrange(
                    g + self._screenGdelta[ix][iy], min=-255
                )
                self._screenBdelta[ix][iy] = getwithinrange(
                    b + self._screenBdelta[ix][iy], min=-255
                )
                self._screendirty[ix][iy] = True
        if self._autoupdate:
            self.update()
    ...
    def putchars(self, chars, x=None, y=None, fgcolor=None, bgcolor=None, indent=False):
        ...
        for ch in chars:
            ...
            if (
                tempcurx >= self._width
                or ch in ("\n", "\r")
                or (iswide(ch) and tempcurx + 1 == self._width)
            ):
    ...
    def write(self, text, x=None, y=None, fgcolor=None, bgcolor=None):
        ...
        for ch in text:
            if ch in (
                "\n",
                "\r",
            ):  # TODO - wait, this isn't right. We should be ignoring one of these newlines. Otherwise \r\n shows up as two newlines.
                self._cursory += 1
                self._cursorx = 0
            else:
    ...
    def _propsetinputcursor(self, value):
        ...
        if x != self._inputcursorx or y != self._inputcursory:
            self._repaintcell(
                self._inputcursorx, self._inputcursory
            )  # blank out the old cursor position
    ...
    def _propsetautodisplayupdate(self, value):
        if self._windowsurface is not None:
            ...
            assert (
                False
            ), "Window Surface object must be set to a surface before autodisplayupdate can be enabled."
    ...
    def _propgetright(self):
        return (
            self._width - 1
        )  # note: this behavior is different from pygame Rect objects, which do not have the -1.
    ...
    def _propgetpixelrect(self):
        return pygame.Rect(
            0, 0, self._width * self._cellwidth, self._height * self._cellheight
        )
    ...
    right = property(
        _propgetright, None
    )  # TODO - need set functions for properties that cause a resize
...
class PygcurseWindow(PygcurseSurface):
    ...
    def __init__(
        ...
        if RUNNING_ON_PYTHON2:
            super(PygcurseWindow, self).__init__(
                width,
                height,
                font,
                fgcolor,
    ...
    def _propsetfullscreen(self, value):
        if value and not self._fullscreen:
            ...
            self._windowsurface = pygame.display.set_mode(
                (self.pixelwidth, self.pixelheight), pygame.FULLSCREEN
            )
...
class PygcurseInput:
    ...
    def __init__(
        ...
        if maxlength is None and pygsurf is None:
            ...
            self._maxlength = (
                (maxlength is not None and maxlength > 0) and (maxlength) or (4094)
            )
        self.cursor = 0
        ...
        self._promptfgcolor = (
            (promptfgcolor is not None) and (getpygamecolor(promptfgcolor)) or (None)
        )
        self._promptbgcolor = (
            (promptbgcolor is not None) and (getpygamecolor(promptbgcolor)) or (None)
        )
        ...
        if (
            pygsurf._pygcurseClass == "PygcurseWindow"
        ):  # TODO - need a better way to identify the object
            self.pygsurface = pygsurf.surface
        elif (
            pygsurf._pygcurseClass == "PygcurseSurface"
        ):  # TODO - need a better way to identify the object
    ...
    def paste(self, text):
        ...
        if not self.insertMode and len(text) + len(self.buffer) > self._maxlength:
            text = text[
                : self._maxlength - len(self.buffer)
            ]  # truncate the pasted text (this is what web browsers do, so I'm copying that behavior)
        ...
        if self.cursor == len(self.buffer):
            ...
            if self.insertMode:
                ...
                self.buffer = (
                    ...
                    + list(text)
                    + self.buffer[self.cursor + len(text) :]
                )
            else:
                self.buffer = (
                    self.buffer[: self.cursor] + list(text) + self.buffer[self.cursor :]
                )
    ...
    def update(self, pygsurfObj=None):
        ...
        if pygsurfObj is not None and pygsurfObj._pygcurseClass in (
            ...
            raise Exception(
                "No PygcurseSurface object specified to draw the PygcurseWindow object to."
            )
        ...
        if self.multiline:
            ...
            pygsurfObj.write(
                self.prompt, fgcolor=self._promptfgcolor, bgcolor=self._promptbgcolor
            )
            pygsurfObj.write(
                "".join(self.buffer) + " ", fgcolor=self._fgcolor, bgcolor=self._bgcolor
            )  # the space at the end is to change the color of the cursor
            afterPromptX, afterPromptY = pygsurfObj.getnthcellfrom(
                self.startx, self.starty, len(self.prompt)
            )
            pygsurfObj.inputcursor = pygsurfObj.getnthcellfrom(
                afterPromptX, afterPromptY, self.cursor
            )
            ...
            if self.eraseBufferSize is not None:
                ...
                while (
                    tempcursorx < pygsurfObj.width
                    and tempcursorx < self.startx + len(self.prompt) + eraseBufferSize
                ):
            ...
            pygsurfObj.putchars(
                (self.prompt + "".join(self.buffer))[:numToPrint],
                self.startx,
                self.starty,
                fgcolor=self._fgcolor,
                bgcolor=self._bgcolor,
            ...
            pygsurfObj.inputcursor = pygsurfObj.getnthcellfrom(
                self.startx, self.starty, self.cursor
            )
    ...
    def sendkeyevent(self, keyEvent):
        ...
        char = interpretkeyevent(keyEvent)
        if (
            char in ("\r", "\n") and keyEvent.type == KEYUP
        ):  # TODO - figure out which is the right one
            self.done = True
            ...
            if char is None and keyEvent.key in self.KEYMAPPING:
                ...
                if (
                    self.whitelistchars is not None and char not in self.whitelistchars
                ) or (self.blacklistchars is not None and char in self.blacklistchars):
                ...
                if (not self.insertMode and len(self.buffer) < self._maxlength) or (
                    ...
                    self.cursor += 1
        self.pygsurf.inputcursor = self.pygsurf.getnthcellfrom(
            self.startx, self.starty, self.cursor
        )
        self.pygsurf.inputcursor = (
            self.pygsurf.inputcursor[0] + len(self.prompt),
            self.pygsurf.inputcursor[1],
...
class PygcurseTextbox:
    def __init__(
        ...
        self.fgcolor = (
            (fgcolor is None) and (pygsurf.fgcolor) or (getpygamecolor(fgcolor))
        )
        self.bgcolor = (
            (bgcolor is None) and (pygsurf.bgcolor) or (getpygamecolor(bgcolor))
        )
        self.text = text
        self.wrap = wrap
        self.border = (
            border  # value is one of 'basic', 'rounded', or a single character
        )
    ...
    def update(self, pygsurf=None):
        ...
        x, y, width, height = pygsurf.getregion(
        ...
        x, y, width, height = pygsurf.getregion(
            (self.x, self.y, self.width, self.height), False
        )
        if (x, y, width, height) == (None, None, None, None):
            return
        ...
        if self.border in ("basic", "rounded"):
            ...
            if pygsurf.isonscreen(x + width - 1, y):
                pygsurf._screenchar[x + width - 1][y] = (
                    (self.border == "basic") and "+" or "\\"
                )
            if pygsurf.isonscreen(x, y + height - 1):
                pygsurf._screenchar[x][y + height - 1] = (
                    (self.border == "basic") and "+" or "\\"
                )
            if pygsurf.isonscreen(x + width - 1, y + height - 1):
                pygsurf._screenchar[x + width - 1][y + height - 1] = (
                    (self.border == "basic") and "+" or "/"
                )
        ...
        if self.shadow is not None:
            pygsurf.addshadow(
                ...
                width=width,
                height=height,
                amount=self.shadowamount,
                direction=self.shadow,
                xoffset=self.shadowxoffset,
                yoffset=self.shadowyoffset,
            )
        ...
        if (
            y < 0
        ):  # if the top row of text is above the top edge of the surface, then truncate
            text = text[abs(y) :]
            y = 0
        maxDisplayedLines = (
            (y + height < pygsurf._height) and (height) or (max(0, pygsurf._height - y))
        )

        truncateLeftChars = (x < 0) and abs(x) or 0
        maxDisplayedLength = (
            (x + width < pygsurf._width) and (width) or (max(0, pygsurf._width - x))
        )
        iy = 0
        for line in text:
...
def regionsoverlap(region1, region2):
    return (
        withinregion(region1[0], region1[1], region2)
        or withinregion(region1[0] + region1[2], region1[1], region2)
        or withinregion(region1[0], region1[1] + region1[3], region2)
        or withinregion(region1[0] + region1[2], region1[1] + region1[3], region2)
        or withinregion(region2[0], region2[1], region1)
        or withinregion(region2[0] + region2[2], region2[1], region1)
        or withinregion(region2[0], region2[1] + region2[3], region1)
        or withinregion(region2[0] + region2[2], region2[1] + region2[3], region1)
    )
...
def withinregion(x, y, region):
    return (
        x > region[0]
        and x < region[0] + region[2]
        and y > region[1]
        and y < region[1] + region[3]
    )
