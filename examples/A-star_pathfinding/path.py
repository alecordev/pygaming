...
class Game:
    ...
    def Convert(self):
        array = [
            [self.grid.nodes[col][row].solid for row in range(self.grid.length)]
    ...
    def blitInfo(self):
        text = self.font.render(
            "Press Enter to find path, press Space to clear board", 1, (255, 255, 255)
        )
        text2 = self.font.render(
            "Press c to toggle checked nodes, and 1 and 2 to switch pathing types",
            1,
            (255, 255, 255),
        )

        check = self.font.render(
            "Checked nodes: " + str(self.show_checked), 1, (255, 255, 255)
        )
        ptype = self.font.render(
            "Pathing type: " + self.pathing_type, 1, (255, 255, 255)
        )
