...
# 5 x 8
test_map = ["xxxxx", "xs..x", "x..ux", "x..dx", "x..rx", "x..ex", "x...x", "xxxxx"]
...
class Grid(object):
    ...
    def get_cell(self, x, y):
        ...
        y = int(y + 0.5)
        return (
            (x - self.xoff + self.dx // 2) // self.dx,
            (y - self.yoff + self.dy // 2) // self.dy,
        )
...
class Mapper(object):
    ...
    def select(self, mode=START):
        ...
        if mode == START:
            ...
            if len(self.maps) > 1:
                self.act_index = random.choice(
                    list(set(range(n)) - set((self.act_index,)))
                )
        else:
        ...
        for y in range(smap.height):
            for x in range(width):
                ...
                if place not in NOT_DRAWABLES:
                    view.rectangle(
                        grid.get_rect(x, y), mapcolors[place], place in PLACES
                    )
...
class Player(object):
    ...
    def vertex_sensors(self):
        ...
        x, y = self.pos
        return [
            (x + sx * self.width, y + sy * self.height) for sx, sy in Player.sensor_pts
        ]
