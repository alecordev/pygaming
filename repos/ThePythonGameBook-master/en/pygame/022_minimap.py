...
class Bullet(pygame.sprite.Sprite):
    ...
    def calculate_heading(self):
        ...
        pygame.draw.rect(
            image, self.color, (0, 0, int(Bullet.side * 1.5), Bullet.side)
        )  # rectangle 1.5 length
        pygame.draw.circle(
            image, self.color, (int(self.side * 1.5), self.side // 2), self.side // 2
        )  #  circle
        pygame.draw.circle(
            image, (0, 0, 0), (int(Bullet.side * 1.5), Bullet.side // 2), 2
        )  # point circle
    ...
    def calculate_origin(self):
        ...
        self.pos[0] += math.cos(degrees_to_radians(self.boss.turretAngle)) * (
            Tank.side - 20
        )
        self.pos[1] += math.sin(degrees_to_radians(-self.boss.turretAngle)) * (
            Tank.side - 20
        )
...
class Tracer(Bullet):
    ...
    def calculate_heading(self):
        ...
        pygame.draw.rect(
            image, (0, 0, 0), (Tracer.side * 0.75, 0, Tracer.side, Tracer.side // 4)
        )  # red dot at front
        image.set_colorkey((128, 128, 128))  # grey transparent
        self.image0 = image.convert_alpha()
        self.image = pygame.transform.rotate(self.image0, self.angle)
    ...
    def calculate_origin(self):
        ...
        if self.turret:
            self.pos[0] += (
                math.cos(degrees_to_radians(-90 + self.boss.turretAngle)) * 15
            )
            self.pos[1] += math.sin(degrees_to_radians(90 - self.boss.turretAngle)) * 15
        else:
            self.pos[0] += math.cos(degrees_to_radians(30 + self.boss.tankAngle)) * (
                Tank.side / 2
            )
            self.pos[1] += math.sin(degrees_to_radians(-30 - self.boss.tankAngle)) * (
                Tank.side / 2
            )
...
class Radarmap(pygame.sprite.Sprite):
    ...
    def __init__(self):
        ...
        self.rect = self.image.get_rect()
        self.rect.topleft = (
            Config.width - Config.radarmapwidth,
            0,
        )  # topleft is a pygame variable
        self.factorx = (
            Config.radarmapwidth * 1.0 / Config.bigmapwidth
        )  # 1.0 to force decimapl point calculation
    ...
    def update(self, seconds):
        ...
        pygame.draw.rect(
            ...
            (255, 255, 255),
            (
                round(Config.cornerpoint[0] * self.factorx, 0),
                round(Config.cornerpoint[1] * self.factory, 0),
                round(Config.width * self.factorx, 0),
                round(Config.height * self.factory, 0),
            ),
            1,
        )
        for tanknumber in Tank.book:  # tank are circles with radius 4
            ...
            pygame.draw.circle(
                self.image,
                color,
                (int(pos[0] * self.factorx), int(pos[1] * self.factory)),
                4,
            )
...
class Tank(pygame.sprite.Sprite):
    ...
    def __init__(self, startpos=(150, 150), angle=0):
        ...
        if self.side > 10:
            pygame.draw.rect(
                image, self.color, (5, 5, self.side - 10, self.side - 10)
            )  # tank body, margin 5
            pygame.draw.rect(
                image, (90, 90, 90), (0, 0, self.side // 6, self.side)
            )  # track left
            pygame.draw.rect(
                image,
                (90, 90, 90),
            ...
            pygame.draw.rect(
                image, (255, 0, 0), (self.side // 6 + 5, 10, 10, 5)
            )  # red bow rect left
            # pygame.draw.rect(image, (255,0,0), (self.side/2 - 5, 10, 10, 5)) # red bow rect middle
        pygame.draw.circle(
            image, (255, 0, 0), (self.side // 2, self.side // 2), self.side // 3, 2
        )  # red circle for turret
    ...
    def update(self, seconds):  # no need for seconds but the other sprites need it
        ...
        # ---------------- rotate tank ---------------
        self.tankAngle += (
            self.tankturndirection * self.tankTurnSpeed * seconds
        )  # time-based turning of tank
        ...
        self.turretAngle += (
            self.tankturndirection * self.tankTurnSpeed * seconds
            + self.turndirection * self.turretTurnSpeed * seconds
        )  # time-based turning
        # ---------- fire cannon -----------
...
class Turret(pygame.sprite.Sprite):
    ...
    def update(self, seconds):
        ...
        if self.boss.firestatus > 0:
            self.image = self.images[
                int(self.boss.firestatus // (Tank.recoiltime / 10.0))
            ]
        else:
    ...
    def draw_cannon(self, offset):
        ...
        image = pygame.Surface(
            (self.boss.side * 2, self.boss.side * 2)
        )  # created on the fly
        image.fill((128, 128, 128))  # fill grey
        pygame.draw.circle(
            image, (255, 0, 0), (self.side, self.side), 22, 0
        )  # red circle
        pygame.draw.circle(
            image, (0, 255, 0), (self.side, self.side), 18, 0
        )  # green circle
        pygame.draw.rect(
            image, (255, 0, 0), (self.side - 10, self.side + 10, 15, 2)
        )  # turret mg rectangle
        pygame.draw.rect(
            image,
            (0, 255, 0),
...
def main():
    ...
    for x in range(
        0, Config.bigmapwidth, Config.bigmapwidth // Config.xtiles
    ):  # start, stop, step
        pygame.draw.line(bigmap, (64, 64, 64), (x, 0), (x, Config.bigmapheight))
    for y in range(
        0, Config.bigmapheight, Config.bigmapheight // Config.ytiles
    ):  # start, stop, step
        pygame.draw.line(bigmap, (64, 64, 64), (0, y), (Config.bigmapwidth, y))
    pygame.draw.rect(
        bigmap, (255, 0, 0), (0, 0, Config.bigmapwidth, Config.bigmapheight), 25
    )  # red bigmap edge
    # paint thin red cross in the middle of the map
    pygame.draw.line(
        bigmap,
        (200, 0, 0),
        (Config.bigmapwidth / 2, 0),
        (Config.bigmapwidth / 2, Config.bigmapheight),
        1,
    )
    pygame.draw.line(
        bigmap,
        (200, 0, 0),
        (0, Config.bigmapheight / 2),
        (Config.bigmapwidth, Config.bigmapheight / 2),
        1,
    )
    ...
    background = bigmap.subsurface(
        (Config.cornerpoint[0], Config.cornerpoint[1], Config.width, Config.height)
    )  # take snapshot of bigmap
    # -----------------------------------
    ...
    while mainloop:
        ...
        pygame.display.set_caption(
            "%s FPS: %.2f playtime: %.1f " % (Config.title, clock.get_fps(), playtime)
        )
        # screen.blit(background, (0,0)) # delete all
        if scrollx == 0 and scrolly == 0:  # only necessery if there was no scrolling
            allgroup.clear(
                screen, background
            )  # funny effect if you outcomment this line
        else:
            background = bigmap.subsurface(
                (
                    Config.cornerpoint[0],
