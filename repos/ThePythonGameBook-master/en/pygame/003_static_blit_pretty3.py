...
class PygView(object):
    def __init__(self, width=640, height=400, fps=30):
        ...
        self.newflash()
        self.ball1 = Ball(x=5, y=5)
        self.ball2 = Ball(x=100, y=200, color=(200, 100, 20))
    ...
    def paint(self):
        ...
        for radius in range(self.width // 2, 4, -10):
            pygame.draw.circle(
                self.screen,
    ...
    def run(self):
        ...
        while running:
            ...
            if pressed[pygame.K_RIGHT]:
                self.ball2.x += 5
            # ----- rand sperre ----
            self.ball1.check(self.width, self.height)
            self.ball2.check(self.width, self.height)
            # ----- ball malen ----
            self.ball1.blit(self.screen)
            self.ball2.blit(self.screen)
            # ----- linie zur maus malen ----
...
class Ball(object):
    ...
    def __init__(self, radius=5, color=(0, 0, 255), x=320, y=240):
    ...
    def check(self, rechts, unten):
        if self.x < 0:
            self.x = 0
        if self.y < 0:
            self.y = 0
        if self.x + self.radius * 2 > rechts:
            self.x = rechts - self.radius * 2
        if self.y + self.radius * 2 > unten:
            self.y = unten - self.radius * 2

    def blit(self, background):
        """blit the Ball on the background"""
        background.blit(self.surface, (self.x, self.y))
        if (self.x + self.radius, self.y + self.radius) not in self.spur:
            self.spur.insert(0, (self.x + self.radius, self.y + self.radius))
        if len(self.spur) > 500:
            self.spur = self.spur[:500]
        # ---- spur malen -----
        start = (self.x + self.radius, self.y + self.radius)
        for p in self.spur:
            pygame.draw.line(background, self.color, start, p, 5)
            start = p
