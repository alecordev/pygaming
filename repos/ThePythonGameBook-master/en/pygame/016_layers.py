...
def game():
    ...
    class Lifebar(pygame.sprite.Sprite):
        ...
        def __init__(self, bossnumber):
            ...
            pygame.draw.rect(
                self.image,
                (0, 255, 0),
                (0, 0, Bird.birds[self.bossnumber].rect.width, 7),
                1,
            )
            self.rect = self.image.get_rect()
            self.oldpercent = 0
        ...
        def update(self, time):
            self.percent = (
                Bird.birds[self.bossnumber].hitpoints
                / Bird.birds[self.bossnumber].hitpointsfull
                * 1.0
            )
            if self.percent != self.oldpercent:
                pygame.draw.rect(
                    self.image,
                    (0, 0, 0),
                    (1, 1, Bird.birds[self.bossnumber].rect.width - 2, 5),
                )  # fill black
                pygame.draw.rect(
                    self.image,
                    (0, 255, 0),
                    (
                        1,
                        1,
                        int(Bird.birds[self.bossnumber].rect.width * self.percent),
                        5,
                    ),
                    0,
                )  # fill green
            ...
            self.rect.centery = (
                Bird.birds[self.bossnumber].rect.centery
                - Bird.birds[self.bossnumber].rect.height / 2
                - 10
            )
    ...
    background.blit(
        write("press right mouse button to decrease Bird's layer."), (50, 65)
    )
    background.blit(
        write("layer of mountains are: -1 (blue), -2 (pink), -3 (red)"), (50, 90)
    )
    ...
    while mainloop:  # ----------------- mainloop ----------------------
        ...
        for event in pygame.event.get():
            if event.type == pygame.QUIT:
                ...
                # Fragment.gravity = not Fragment.gravity # toggle gravity class variable
                    ...
                    Fragment.gravity = (
                        not Fragment.gravity
                    )  # toggle gravity class variable
            ...
            if pygame.mouse.get_pressed()[0]:
                if birdlayer < 10:
                    ...
                    for bird in birdgroup:
                        allgroup.change_layer(
                            bird, birdlayer
                        )  # allgroup draws the sprite
                    for bar in bargroup:
                        allgroup.change_layer(
                            bar, birdlayer
                        )  # allgroup draws the sprite
        ...
        pygame.display.set_caption(
            "fps: %.2f birds: %i grav: %s"
            % (clock.get_fps(), len(birdgroup), Fragment.gravity)
        )

        birdtext.newmsg(
            "current Bird _layer = %i" % birdlayer
        )  # update text for birdlayer
        ...
        crashgroup = pygame.sprite.spritecollide(
            hunter, birdgroup, False, pygame.sprite.collide_circle
        )
        ...
        for bird in birdgroup:  # test if a bird collides with another bird
            ...
            for crashbird in crashgroup:
                if (
                    crashbird.number != bird.number
                ):  # different number means different birds
                    bird.crashing = True
                    if not bird.waiting:
