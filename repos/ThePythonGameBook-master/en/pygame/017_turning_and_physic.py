...
def game(folder="data"):
    ...
    FRICTION = 0.991  # between 1 and 0. 1 means no friction at all (deep space)
    ...
    class Lifebar(pygame.sprite.Sprite):
        ...
        def paint(self):
            ...
            pygame.draw.rect(
                self.image, (0, 255, 0), (0, 0, self.boss.rect.width, 7), 1
            )
            self.rect = self.image.get_rect()
    ...
    class Bird(pygame.sprite.Sprite):
        ...
        def __init__(self, layer=4, bigbird=False):
            ...
            print(
                "my number %i Bird number %i and i am a %s "
                % (self.number, Bird.number, getclassname(self))
            )
            self.mass = 100.0
            self.angle = 0.0
        ...
        def areacheck(self):
            if not self.area.contains(self.rect):
                ...
                if self.pos[1] + self.rect.height / 2 > self.area.bottom:
                    ...
                    self.boosttime = self.boostmin + random.random() * (
                        self.boostmax - self.boostmin
                    )
        ...
        def update(self, seconds):
            ...
            if self.waiting:
                ...
                self.image = Bird.image[
                    self.crashing + self.big
                ]  # 0 for not crashing, 1 for crashing
                self.image0 = Bird.image[
                    self.crashing + self.big
                ]  # 0 for not crashing, 1 for crashing
    ...
    class BlueFragment(Fragment):
        ...
        def __init__(self, pos):
            ...
            self.lifetime = (
                Bird.waittime + random.random() * 0.5
            )  # a bit more livetime after the Bird appears
            self.init2()
    ...
    try:  # load images into classes (class variable !). if not possible, draw ugly images
        ...
        print(
            "no image files 'babytux.png' and 'babytux_neg.png' in subfolder %s"
            % folder
        )
    ...
    Bird.image.append(
        pygame.transform.scale2x(Bird.image[0])
    )  # copy of first image, big bird
    Bird.image.append(
        pygame.transform.scale2x(Bird.image[1])
    )  # copy of blue image, big bird
    ...
    try:  # ------- load sound -------
        ...
        print(
            "could not load one of the sound files from folder %s. no sound, sorry"
            % folder
        )
    ...
    while mainloop:
        ...
        for event in pygame.event.get():
            if event.type == pygame.QUIT:
                ...
                ):  # get sprites at mouse position, print info
                    print("=========================")
                    print("-----Spritelist---------")
        ...
        pygame.display.set_caption(
            "fps: %.2f gravity: %s hits:%i shots:%i quota:%.2f%%"
            % (clock.get_fps(), gravity, hits, player.shots, quota)
        )
        # ------ collision detection
                ...
                for crashbird in crashgroup:  # test bird with other bird collision
                    if crashbird.number > bird.number:  # avoid checking twice
                        ...
                        if not (bird.waiting or crashbird.waiting):
                            elastic_collision(
                                crashbird, bird
                            )  # change dx and dy of both birds
        ...
        if (player.hitpoints < 1 or playtime > gametime) and not gameOver:
            ...
            screentext.newmsg(
                "Game Over. hits/shots: %i/%i quota: %.2f%%"
                % (hits, player.shots, quota),
                (255, 0, 0),
            )
