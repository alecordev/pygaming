...
def play_the_game():
    ...
    def cleanblit(sprite, x, y):
        ...
        dirtyrect = background.subsurface(
            (
                round(x - sprite.get_width() / 2, 0),
    ...
    background.blit(
        write("control the snake with the cursor keys (Enter to stop)"), (10, 10)
    )
    background.blit(
        write("control the bird with the mouse (left button to stop)"), (10, 30)
    )
    background.blit(
        write("the cross is always in the middle between snake and bird"), (10, 50)
    )
    background.blit(write("the blue circle (police) moves toward the cross"), (10, 70))
    background.blit(
        write("catch the red triangle with the blue circle to win points"), (10, 90)
    )
    ...
    while mainloop:
        ...
        pygame.display.set_caption(
            ...
            % (clock.get_fps(), snakedx, snakedy, birddx, birddy, policedx, policedy)
        )
        if playtime < 0:
            gameOver = True
        if gameOver:
            ...
            crossx = (
                ...
                + (
                    max(birdx, snakex)
                    - min(birdx, snakex)  # cross is in the middle of bird and snake
                )
                / 2.0
                - cross.get_width() / 2
            )
            crossy = (
                min(birdy, snakey)
                + (max(birdy, snakey) - min(birdy, snakey)) / 2.0
                - cross.get_height() / 2
            )
            if crossx < policex:
            ...
            # ----------- bounce ----------
            policex, policey, policedx, policedy = bounce(
                police, policex, policey, policedx, policedy
            )
            birdx, birdy, birddx, birddy = bounce(bird, birdx, birdy, birddx, birddy)
            snakex, snakey, snakedx, snakedy = bounce(
                snake, snakex, snakey, snakedx, snakedy
            )
            thiefx, thiefy, thiefdx, thiefdy = bounce(
                thief, thiefx, thiefy, thiefdx, thiefdy
            )
            # --- police got thief ? -----
            distx = max(
                policex + police.get_width() / 2, thiefx + thief.get_width() / 2
            ) - min(policex + police.get_width() / 2, thiefx + thief.get_width() / 2)
            disty = max(
                policey + police.get_height() / 2, thiefy + thief.get_height() / 2
            ) - min(policey + police.get_width() / 2, thiefy + thief.get_width() / 2)
            ...
            pygame.draw.line(
                screen, randomcolour(), (snakex, snakey), (birdx, birdy), 1
            )
            pygame.draw.line(
                screen, randomcolour(), (crossx, crossy), (policex, policey), 1
            )
