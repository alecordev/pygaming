...
class AlphaDemo(object):
    def __init__(self, width=900, height=600, fontsize=24):
        ...
        self.glob_alphas = itertools.cycle(
            itertools.chain(itertools.chain(alpha_up, alpha_down))
        )
        self.step = 4
        self.mode_nr = 7
    ...
    def action(self, pressed_keys):
        ...
        self.show_surfaces(
            self.jpg_monster, "jpg", 0, 300, 200, 180, glob_alpha, self.pp_rgba, mode
        )

        text = (
            "left/right=red>%d  comma/period=green>%d  up/dwn=blue>%d  "
            "+/-=ppalpha>%d  " % tuple(self.pp_rgba)
        )
        pygame.display.set_caption("%s  Enter: Mode>%s" % (text, mode_text))

    def show_surfaces(
        self, surf, pictype, x, y, x_delta, height, glob_alpha, pp_rgba, mode
    ):
