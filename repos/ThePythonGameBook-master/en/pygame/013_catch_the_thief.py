...
def intro(screen):
    ...
    screen.blit(
        write("control the snake with the cursor keys or WASD (Enter or LCTRLto stop)"),
        (10, 70),
    )
    screen.blit(
        write("control the bird with the mouse (left button to stop)"), (10, 90)
    )
    screen.blit(
        write("the cross is always in the middle between snake and bird"), (10, 110)
    )
    screen.blit(write("the blue circle (police) moves toward the cross"), (10, 130))
    screen.blit(
        write("catch the red triangle (the thief) with the blue circle to win points"),
        (10, 150),
    )
...
def draw(sprite, x, y):
    ...
    Config.screen.blit(
        sprite,
        (round(x, 0) - sprite.get_width() / 2, round(y, 0) - sprite.get_height() / 2),
    )
...
def play_the_game():
    ...
    try:
        ...
        raise (
            UserWarning,
            "Unable to find or play the files in the folder 'data' :-( ",
        )
    ...
    Config.screen = (
        screen  # copy screen into the Config class so that functions can access screen
    )
    ...
    pygame.draw.rect(
        background,
        (200, 200, 200),
        ((screen.get_width() - 360, screen.get_height() - 25), (360, 25)),
    )
    ...
    while mainloop:
        ...
        if gameOver:
            ...
            screen.blit(
                write(
                    "Game Over. %.2f points. Press ESCAPE" % points, (128, 0, 128), 64
                ),
                (20, 250),
            )
        else:
            screen.blit(
                write("points: %.2f time left: %.2f seconds" % (points, playtime)),
            ...
            # ------------ compute movement ----------------
            Config.crossx = (
                min(Config.birdx, Config.snakex)
                + (
                    max(Config.birdx, Config.snakex)
                    - min(  # cross is in the middle of bird and snake
                        Config.birdx, Config.snakex
                    )
                )
                / 2.0
                - cross.get_width() / 2
            ...
            Config.crossy = (
                min(Config.birdy, Config.snakey)
                + (max(Config.birdy, Config.snakey) - min(Config.birdy, Config.snakey))
                / 2.0
                - cross.get_height() / 2
            )
            ...
            if Config.crossy > Config.policey:
                ...
                Config.policedy -= 1
            Config.thiefdx += random.randint(
                -Config.erratic, Config.erratic
            )  # thief is erratic
            Config.thiefdy += random.randint(-Config.erratic, Config.erratic)
            Config.thiefdx = max(
                Config.thiefdx, -Config.thiefmaxspeed
            )  # limit speed of thief
            ...
            # ----------- bounce ----------
            Config.policex, Config.policey, Config.policedx, Config.policedy = bounce(
                police, Config.policex, Config.policey, Config.policedx, Config.policedy
            )
            Config.birdx, Config.birdy, Config.birddx, Config.birddy = bounce(
                bird, Config.birdx, Config.birdy, Config.birddx, Config.birddy
            )
            Config.snakex, Config.snakey, Config.snakedx, Config.snakedy = bounce(
                snake, Config.snakex, Config.snakey, Config.snakedx, Config.snakedy
            )
            Config.thiefx, Config.thiefy, Config.thiefdx, Config.thiefdy = bounce(
                thief, Config.thiefx, Config.thiefy, Config.thiefdx, Config.thiefdy
            )
            # --- police got thief ? collision detection -----
            distx = max(
                Config.policex + police.get_width() / 2,
                Config.thiefx + thief.get_width() / 2,
            ) - min(
                Config.policex + police.get_width() / 2,
                Config.thiefx + thief.get_width() / 2,
            )
            disty = max(
                Config.policey + police.get_height() / 2,
                Config.thiefy + thief.get_height() / 2,
            ) - min(
                Config.policey + police.get_width() / 2,
            ...
            pygame.draw.line(
                screen,
                randomcolour(),
                (Config.snakex, Config.snakey),
                (Config.birdx, Config.birdy),
                1,
            )
            pygame.draw.line(
                screen,
                randomcolour(),
                (Config.crossx, Config.crossy),
                (Config.policex, Config.policey),
                1,
            )
