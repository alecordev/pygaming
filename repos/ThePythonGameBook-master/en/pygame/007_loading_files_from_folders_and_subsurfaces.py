...
try:  # try to load images from the harddisk
    prettybackground = pygame.image.load(
        os.path.join(folder, "800px-La_naissance_de_Venus.jpg")
    )
    uglybackground = pygame.image.load(os.path.join(folder, "background800x470.jpg"))
    snakesurface = pygame.image.load(
        os.path.join(folder, "snake.gif")
    )  # with tranparent colour
except:
    ...
    msg += (
        "see http://thepythongamebook.com/en:part2:pygame:step007 for more information"
    )
...
prettybackground = (
    prettybackground.convert()
)  # convert (no alpha! because no tranparent parts) for faster blitting
...
screen.blit(
    snakesurface, (x, y)
)  # blit the ball surface on the screen (on top of background)
...
while mainloop:
    ...
    pygame.display.set_caption(
        "FPS: {:.2f} dx:{} dy:{} [p]aint ({}) "
        "paint, [d]irtyrect ({}), [r]estore".format(
            clock.get_fps(), dx, dy, painting, dirty
        )
    )
    ...
    try:
        tvscreen = prettybackground.subsurface(
            (x, y, snakerect.width, snakerect.height)
        )
    except:
