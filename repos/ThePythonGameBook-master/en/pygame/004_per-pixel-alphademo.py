...
class PeepDemo(object):
    def __init__(self, **opts):
        ...
        self.pic_offset = (
            offset(self.width, self.pic.get_width()),
    ...
    def calc_rad_alphas(self, radius, n):
        ...
        alpha_step = 256 // n
        self.rad_alphas = [
            (radius - i * rad_step, 255 - i * alpha_step) for i in xrange(n)
        ]
        ...
        while mainloop:
            ...
            for event in pygame.event.get():
                if event.type == pygame.QUIT:
                    ...
                    self.calc_centers(
                        self.center, pygame.mouse.get_pos(), self.hole_count
                    )
                elif event.type == pygame.MOUSEBUTTONDOWN:
                    # check mouse wheel
                    if event.button in (4, 5):
                        self.hole_count = check(
                            self.hole_count + [-1, 1][event.button - 4], 2, 64
                        )
                        self.calc_rad_alphas(self.max_radius, self.hole_count)
                        self.calc_centers(
                            self.center, pygame.mouse.get_pos(), self.hole_count
