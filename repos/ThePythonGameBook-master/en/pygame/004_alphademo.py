...
def get_alpha_surface(
    surf, alpha=128, red=128, green=128, blue=128, mode=pygame.BLEND_RGBA_MULT
):
...
def alphademo(width=800, height=600):
    ...
    venus = pygame.image.load(
        os.path.join("data", "800px-La_naissance_de_Venus.jpg")
    ).convert()
    ...
    pngMonster = pygame.image.load(
        os.path.join("data", "colormonster.png")
    ).convert_alpha()
    ...
    while mainloop:
        ...
        pygame.display.set_caption(
            "insert/del=red:%i, home/end=green:%i, pgup/pgdwn=blue:%i, +/-=pixalpha:%i press ESC"
            % (r, g, b, a)
        )
        ...
        dr, dg, db, da = (
            ...
            0,
            0,
        )  # set changing to 0 for red, green, blue, pixel-alpha
        ...
        screen.blit(
            write(
                "press [INS] / [DEL] to change red value: %i" % r, 24, (255, 255, 255)
            ),
            (190, 150),
        )
        screen.blit(
            write("press [HOME] / [END] to change green value: %i" % g), (190, 170)
        )
        screen.blit(
            write("press [PgUp] / [PgDwn] to chgange blue value: %i" % b), (190, 190)
        )
        screen.blit(
            write("press [Enter] for mode: %i (%s)" % (mode, modelist[modeNr])),
            (190, 230),
        )
        screen.blit(
            write("press [+] / [-] (Keypad) to chgange alpha value: %i" % a), (190, 210)
        )
        # ------ next frame --------
