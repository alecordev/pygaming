...
def game(folder="data"):
    ...
    screen = pygame.display.set_mode(
        (1800, 1000)
    )  # try out larger values and see what happens !
    ...
    class Lifebar(pygame.sprite.Sprite):
        ...
        def __init__(self, boss):
            ...
            self.oldangle = (
                self.boss.angle
            )  # store angle of boss to redraw bar if boss is rotating
        ...
        def update(self, time):
            ...
            if (self.percent != self.oldpercent) or (self.oldangle != self.boss.angle):
                ...
                pygame.draw.rect(
                    self.image, (0, 0, 0), (1, 1, self.boss.rect.width - 2, 5)
                )  # fill black
                pygame.draw.rect(
                    self.image,
                    self.color,
                    (1, 1, int(self.boss.rect.width * self.percent), 5),
                    0,
                )  # fill green
            self.oldpercent = self.percent
            self.rect.centerx = self.boss.rect.centerx
            self.rect.centery = (
                self.boss.rect.centery - self.boss.rect.height / 2 - self.distance
            )
    ...
    class GameObject(pygame.sprite.Sprite):
        ...
        gameobjects = (
            {}
        )  # a dictionary of all GameObjects, each GameObject has its own number
        number = 0
        ...
        def __init__(
            ...
            self.bouncefriction = (
                -0.5
            )  # how much speed is lost by bouncing off a wall. 1 means no loss, 0 means full stop
            ...
            GameObject.gameobjects[
                self.number
            ] = self  # store myself into the GameObject dictionary
    ...
    class Player(GameObject):
        ...
        def __init__(self, playernumber=0):
            ...
            GameObject.__init__(
                self, areastop=True, angle=self.angle, speedmax=300
            )  # ------------------- important ! ----------------------
            ...
            self.cooldown = 0.0
            self.rocketcooldowntime = 0.005  # seconds
        ...
        def get_target_nr(self):
            ...
            if Player.duel:
                if (GameObject.gameobjects[0] is not None) and (
                    GameObject.gameobjects[1] is not None
                ):
        ...
        def update(self, seconds):
            ...
            if self.cooldown > 0:  # ------ can not shoot
                ...
                if (self.playernumber == 1 and pressedkeys[pygame.K_KP0]) or (
                    self.playernumber == 0 and pressedkeys[pygame.K_SPACE]
                ):  # shoot forward
                    ...
                    if self.rocketcooldown > 0:
                        ...
                        if self.rockets > self.rocketsmax / 2:  # heavy sliding rocket
                            if self.targetnumber is not None:
                                crysound.play()
                                Rocket(
                                    self, self.targetnumber, 1, -30
                                )  # boss, target, type, launchangle
                                Rocket(
                                    self, self.targetnumber, 1, 30
                                )  # boss, target, type, launchangle
    ...
    class Monster(GameObject):
        ...
        def __init__(self, pos=screenrect.center):
            ...
            self.bulletlifetime = (
                2.8  # longer lifetime than player's bullet, but no lifestealing effect
            )
        ...
        def choose_target_nr(self):
            ...
            if (
                GameObject.gameobjects[0] is not None
                and GameObject.gameobjects[1] is not None
            ):  # both players alive
        ...
        def update(self, seconds):
            ...
            if GameObject.gameobjects[self.targetnumber] is not None:
                ...
                self.phase = self.phases[
                    int(self.alivetime) % 4
                ]  # fully cycle throug all phases each 3 seconds
                if self.phase == "nothing":
                    ...
                    if random.randint(1, 2) == 1:
                        Bullet(
                            self,
                            GameObject.rotate_toward_moving(
                                self, self.targetdistancex, self.targetdistancey
                            ),
                            self.max_abberation,
                        )
                        self.bullets_fired += 1
    ...
    class Fragment(pygame.sprite.Sprite):
        ...
        def init2(
            self,
        ):  # split the init method into 2 parts for better access from subclasses
            ...
            self.rect.center = (
                self.pos
            )  # if you forget this line the sprite sit in the topleft corner
            self.time = 0.0
    ...
    class Wound(Fragment):
        ...
        def __init__(self, pos, greenmin=200, greenmax=255):
            ...
            self.color = (
                ...
                random.randint(self.greenmin, self.greenmax),
                random.randint(0, 50),
            )
            self.groups = allgroup
    ...
    class Smoke(Fragment):
        ...
        def __init__(self, pos, dx, dy, colmin=1, colmax=50):
            ...
            self.smokearc = 0.3  # 0 = thin smoke stream, 1 = 180 Degrees
    ...
    class Bullet(GameObject):
        ...
        def __init__(self, boss, myangle=None, max_abberation=5.5):
            ...
            self.abberation = random.uniform(
                -max_abberation, max_abberation
            )  # no shot is perfect
    ...
    class Rocket(GameObject):
        ...
        def __init__(self, boss, targetnr, type=1, launchangle=0):
            ...
            if type == 1:  # --------heavy sliding missile ----------
                ...
                self.speed = 50
                self.starttime = (
                    0.9  # missles are launched but are not homing in this time
                )
    ...
    background.blit(
        write("red player:  w,a,s,d,q,e fire: SPACE", (130, 130, 130)), (50, 110)
    )
    background.blit(
        write("blue player: Numpad 8,4,5,6,7,9 fire: 0", (130, 130, 130)), (50, 140)
    )
    background.blit(
        write("icrease # of rockets by not firing", (130, 130, 130)), (50, 170)
    )
    background.blit(
        write("ESC=quit, m=new monster o=more overtime", (130, 130, 130)), (50, 200)
    )
    ...
    try:
        Player.image.append(
            pygame.image.load(os.path.join(folder, "player_red2.png")).convert_alpha()
        )  # 0
        Player.image.append(
            pygame.image.load(os.path.join(folder, "player_blue2.png")).convert_alpha()
        )  # 1
        Monster.image.append(
            pygame.image.load(os.path.join(folder, "xmonster_s.png")).convert_alpha()
        )  # 0
        Monster.image.append(
            pygame.image.load(
                os.path.join(folder, "xmonster_fire_s.png")
            ).convert_alpha()
        )  # 1
        Monster.image.append(
            pygame.image.load(
                os.path.join(folder, "xmonster_left_s.png")
            ).convert_alpha()
        )  # 2
        Monster.image.append(
            pygame.image.load(
                os.path.join(folder, "xmonster_right_s.png")
            ).convert_alpha()
        )  # 3
        ...
        raise (
            UserWarning,
            "Sadly i could not loading all graphic or sound files from %s" % folder,
        )
    ...
    overtime = (
        15  # time in seconds to admire the explosion of player before the game ends
    )
    ...
    while mainloop:
        ...
        for event in pygame.event.get():
            if event.type == pygame.QUIT:
                ...
                if event.key == pygame.K_ESCAPE:
                    ...
                    if gameOver:
                        overtime += (
                            10  # more overtime to watch monsters fight each other
                        )
        # ---- new Monster ?
        ...
        for (
            ...
            playergroup
        ):  # test if a player crash into enemy bullet ... vamipr health stealing effect !
            crashgroup = pygame.sprite.spritecollide(
                player, bulletgroup, False, pygame.sprite.collide_mask
            )
            for bullet in crashgroup:  # this include friendly fire
                if (
                    bullet.boss.playernumber != player.playernumber
                ):  # only care for unfriendly fire
            ...
            # player vs player
            crashgroup = pygame.sprite.spritecollide(
                player, playergroup, False, pygame.sprite.collide_circle
            )
            ...
            crashgroup = pygame.sprite.spritecollide(
                player, rocketgroup, False, pygame.sprite.collide_mask
            )
            for rocket in crashgroup:
                # if projectile.physicnumber > crashbody.physicnumber: #avoid checking twice
                if (
                    rocket.boss.playernumber != player.playernumber
                ):  # avoid friendly fire
                    impactsound.play()
        ...
        if GameObject.gameobjects[0] is None and GameObject.gameobjects[1] is None:
            ...
            screentext2.newmsg(
                "Duel mode. Both Players, fight each other!", (255, 0, 255)
            )
        elif len(monstergroup) == 1:
            Player.duel = False
        ...
        if gameOver:  # overtime to watch score, explosion etc
            ...
            screentext3.newmsg(
                "Red player: bullets: %i hit: %i quota: %.2f%% rockets: %i hits: %i quota: %.2f%%"
                % (
                    player1.bullets_fired,
                    player1.bullets_hit,
                    player1.bullets_hit * 100.0 / max(1, player1.bullets_fired),
                    player1.rockets_fired,
                    player1.rockets_hit,
                    player1.rockets_hit * 100.0 / max(1, player1.rockets_fired),
                ),
                (255, 0, 0),
            )
            screentext4.newmsg(
                "Blue player: bullets: %i hit: %i quota: %.2f%% rockets: %i hits: %i quota: %.2f%%"
                % (
                    player2.bullets_fired,
                    player2.bullets_hit,
                    player2.bullets_hit * 100.0 / max(1, player2.bullets_fired),
                    player2.rockets_fired,
                    player2.rockets_hit,
                    player2.rockets_hit * 100.0 / max(1, player2.rockets_fired),
                ),
                (0, 0, 255),
            )
