...
def write(background, text, x=50, y=150, color=(0, 0, 0), fontsize=None, center=False):
...
class FlyingObject(pygame.sprite.Sprite):
    ...
    def create_image(self):
        ...
        pygame.draw.polygon(
            self.image,
            self.color,
            [(0, 0), (2 * self.radius, self.radius), (0, 2 * self.radius),],
        )
    ...
    def wallcheck(self):
        ...
        if self.kill_on_edge:
            if (
                ...
                or self.pos[1] + self.height // 2 > 0
                or self.pos[0] + self.width // 2 > PygView.width
                or self.pos[1] - self.height // 2 < -PygView.height
            ):
...
class Fragment(FlyingObject):
    def _overwrite_parameters(self):
        ...
        self.color = (random.randint(128, 255), random.randint(0, 90), 0)
...
class Rocket(FlyingObject):
    def _overwrite_parameters(self):
        ...
        self.pos = pygame.math.Vector2(self.mothership.pos[0], self.mothership.pos[1])
    ...
    def create_image(self):
        ...
        pygame.draw.polygon(
            self.image, self.color, [(0, 0), (15, 0), (20, 5), (15, 10), (0, 10)]
        )
        pygame.draw.polygon(
            self.image, (150, 150, 150), [(5, 2), (10, 2), (15, 5), (10, 8), (5, 8)]
        )
        self.image.set_colorkey((0, 0, 0))
...
class Rocket2(FlyingObject):
    ...
    def create_image(self):
        ...
        pygame.draw.polygon(
            self.image, self.color, [(0, 0), (0, 15), (20, 15), (25, 10), (20, 0)]
        )
...
class PygView:
    ...
    def preparesprites(self):
        ...
        self.player1 = Spaceship(
            ...
            bounce_on_edge=True,
            control_method="wasd",
            has_hitpointbar=True,
            hitpoints=105,
        )  # creating the Ball object
        self.player2 = Spaceship3(
            x=400,
            y=200,
            bounce_on_edge=True,
            control_method="ijkl",
    ...
    def paint(self):
        ...
        PygView.background1 = pygame.transform.scale(
            PygView.background1, (PygView.width, PygView.height)
        )
        PygView.background2 = pygame.transform.scale(
            PygView.background2, (PygView.width, PygView.height)
        )
        PygView.background3 = pygame.transform.scale(
            PygView.background3, (PygView.width, PygView.height)
        )
        PygView.background4 = pygame.transform.scale(
            PygView.background4, (PygView.width, PygView.height)
        )
        ...
        while running:
            ...
            if left:
                for f in range(random.randint(10, 20)):
                    Fragment(x=pygame.mouse.get_pos()[0], y=pygame.mouse.get_pos()[1])
            if right:
                Flytext(pygame.mouse.get_pos()[0], pygame.mouse.get_pos()[1], "hallo")

            # ---- collision detection----

            for ship in self.shipgroup:
                crashgroup = pygame.sprite.spritecollide(
                    ship, self.rocketgroup, False, pygame.sprite.collide_circle
                )
            ...
            pygame.display.set_caption(
                "player1: HP: {} player 2: HP {} player 3: HP {} ".format(
                    self.player1.hitpoints,
                    self.player2.hitpoints,
                    self.player3.hitpoints,
                )
            )
            # --- paint tails ----
            # for ship in self.shipgroup:
