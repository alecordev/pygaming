...
class Tank(pygame.sprite.Sprite):
    ...
    def __init__(self, startpos=(150, 150), angle=0):
        ...
        Text(
            (Config.width / 2, 30 + 20 * self.number), self.msg
        )  # create status line text sprite
...
def main():
    ...
    for y in range(
        0, Config.height, Config.height / Config.ytiles
    ):  # start, stop, step
    ...
    status3 = Text(
        (Config.width / 2, 10),
        "difference angle (from blue to yellow): %i" % Tank.book[1].aim_at_player(),
    )
    mainloop = True
    while mainloop:
        status3.changemsg(
