...
class Shape:
    ...
    def __init__(
        self, screen, startpoint, pointlist, zoom=1, angle=0, color=(255, 0, 0), width=1
    ):
...
class Ball:
    ...
    def __init__(
        ...
        self.screen = screen
        self.startpoint = Vec2d(
            startpoint.x, startpoint.y
        )  # make a copy of the startpoint vector
        self.move = move
        self.age = 0
        ...
        pygame.draw.circle(
            self.surface, color, (radius, radius), radius
...
class PygView(object):
    def __init__(self, width=1440, height=850, fps=30):
        ...
        pygame.display.set_caption(
            "Press ESC to quit. keys: wasd left_shift,   ijkl right_shift, 12"
        )
        self.width = width
        self.height = height
    ...
    def paint(self):
        ...
        # yannik = Vec2d(-15,32)
        # print("yannik", yannik)
        # yannik2 = Vec2d(yannik.x, yannik.y)
        # print("y2", yannik2)
        # print("y2 x 3")
        # yannik2 *= 3
        # print("yannik", yannik)
        ...
        # jojo draw
        # self.africa = Shape(self.screen, Vec2d(50,50),
        #                               (Vec2d(0,0),
        #                                Vec2d(100,10),
        #                                Vec2d(10,150),
        #                                Vec2d(0,0)
        ...
        self.pixelhirn = Shape(
            self.screen,
            Vec2d(self.width - 100, self.height - 100),
            (
                Vec2d(0, 0),
                Vec2d(-50 // 2, 50 // 2),
                Vec2d(150 // 2, 0 // 2),
    ...
    def run(self):
        ...
        while running:
            for event in pygame.event.get():
                if event.type == pygame.QUIT:
                    ...
                    if event.key == pygame.K_RIGHT:
                        self.africa.startpoint = Vec2d(
                            self.africa.startpoint.x + 20, self.africa.startpoint.y
                        )
            ...
            if pressed[pygame.K_w]:
                self.yanniks_ship.forward(15)
            if pressed[pygame.K_s]:
                self.yanniks_ship.forward(-5)
            ...
            if pressed[pygame.K_i]:
                self.pixelhirn.forward(15)
            if pressed[pygame.K_k]:
                self.pixelhirn.forward(-5)
            ...
            for b in Ball.group:
                if b.bossnumber != self.pixelhirn.number:
                    if (
                        b.startpoint - self.pixelhirn.startpoint
                    ).get_length() < critical_distance:
                        self.pixelhirn.hitpoints -= 1
                if b.bossnumber != self.yanniks_ship.number:
                    if (
                        b.startpoint - self.yanniks_ship.startpoint
                    ).get_length() < critical_distance:
            ...
            pygame.draw.line(
                self.screen,
                (0, 0, 0),
                (self.yanniks_ship.startpoint.x, self.yanniks_ship.startpoint.y),
                (
            ...
            pygame.draw.line(
                self.screen,
                (0, 0, 0),
                (self.pixelhirn.startpoint.x, self.pixelhirn.startpoint.y),
                (self.pixelhirn.startpoint.x + d.x, self.pixelhirn.startpoint.y + d.y),
                8,
            )
            ...
            if pressed[pygame.K_LSHIFT]:
                ...
                Ball(
                    self.screen,
                    self.yanniks_ship.startpoint,
                    c,
                    color=(200, 20, 0),
                    bossnumber=self.yanniks_ship.number,
                )
            ...
            if pressed[pygame.K_RSHIFT]:
                ...
                Ball(
                    self.screen,
                    self.pixelhirn.startpoint,
                    d,
                    color=(0, 0, 200),
                    bossnumber=self.pixelhirn.number,
                )
            ...
            text_time = "FPS: {:4.3} TIME: {:6.3} sec".format(
                self.clock.get_fps(), self.playtime
            )
            text_player1 = "Player1: {}° {} hp".format(
                self.yanniks_ship.angle % 360, self.yanniks_ship.hitpoints
            )
            text_player2 = "Player2: {}° {} hp".format(
                self.pixelhirn.angle % 360, self.pixelhirn.hitpoints
            )
            self.draw_text(text_player1, x=50, y=30, color=(0, 200, 0))
            self.draw_text(
                text_time, x=self.width // 2 - 200, y=30, color=(100, 0, 100)
            )
            self.draw_text(text_player2, x=self.width - 300, y=30, color=(0, 0, 200))
            # self.draw_text("FPS: {:4.3} TIME: {:6.3} sec Player1: {}° {} hp Player2: {}° {} hp".format(
            #               self.clock.get_fps(), self.playtime, self.yanniks_ship.angle % 360, self.yanniks_ship.hitpoints, self.pixelhirn.angle % 360, self.pixelhirn.hitpoints ))
