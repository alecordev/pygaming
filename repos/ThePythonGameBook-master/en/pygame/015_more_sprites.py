...
def game():
    ...
    class Fragment(pygame.sprite.Sprite):
        ...
        def __init__(self, pos):
            ...
            self.image.set_colorkey((0, 0, 0))  # black transparent
            pygame.draw.circle(
                self.image, (random.randint(1, 64), 0, 0), (5, 5), random.randint(2, 5)
            )
            self.image = self.image.convert_alpha()
            self.rect = self.image.get_rect()
            self.rect.center = (
    ...
    class Bird(pygame.sprite.Sprite):
        ...
        def __init__(self, startpos=screen.get_rect().center):
            ...
            self.hitpointsfull = float(
                HITPOINTS
            )  # maximal hitpoints , float makes decimal
    ...
    try:
        ...
        raise (
            UserWarning,
            "no image files 'babytux.png' and 'babytux_neg.png' in subfolder 'data'",
        )
    ...
    while mainloop:
        ...
        if pygame.mouse.get_pressed()[2]:
            ...
            crashgroup = pygame.sprite.spritecollide(
                hunter, birdgroup, True, pygame.sprite.collide_circle
            )
        pygame.display.set_caption(
            "ms: %i max(ms): %i fps: %.2f birds: %i gravity: %s bad:%s clever:%s"
            % (
                milliseconds,
                millimax,
                clock.get_fps(),
        ...
        for bird in birdgroup:
            if not clevercoding:
                if badcoding:
                    othergroup = (
                        birdgroup.copy()
                    )  # WRONG ! THIS CODE MAKES UGLY TIME-CONSUMING GARBAGE COLLECTION !
                else:
                    othergroup[
                        :
                    ] = birdgroup.sprites()  # correct. no garbage collection
                othergroup.remove(
                    bird
                )  # remove the actual bird, only all other birds remain
                ...
                for crashbird in crashgroup:
                    if crashbird.number != bird.number:  # avoid collision with itself
                        ...
                        bird.dx -= (
                            crashbird.pos[0] - bird.pos[0]
                        )  # move bird away from other bird
                        bird.dy -= crashbird.pos[1] - bird.pos[1]
