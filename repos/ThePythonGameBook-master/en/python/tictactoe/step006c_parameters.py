...
def check_win(char: object, cells: object) -> object:
    ...
    for row in range(3):  # checking rows
        if cells[row][0:3] == [
            ...
            char,
            char,
        ]:  # horizontal slice, see code discussion
            return True
...
def choose_a_free_cell(
    cells: List[List[str]], my_char: str, ai: str, silent: bool = False
) -> str:
    ...
    for y, row in enumerate(cells):
        for x, char in enumerate(row):
            ...
            if y == 0 and x == 0:  # topleft
                if any(
                    [
                        (cells[1][1] == mychar and cells[2][2] == mychar),
                ...
                if any(
                    [
                        (cells[1][1] == mychar and cells[2][0] == mychar),
                ...
                if any(
                    [
                        (cells[1][1] == mychar and cells[0][2] == mychar),
                ...
                if any(
                    [
                        (cells[1][1] == mychar and cells[0][0] == mychar),
                ...
                if any(
                    [
                        (cells[0][0] == mychar and cells[0][2] == mychar),
                ...
                if any(
                    [
                        (cells[1][0] == mychar and cells[1][1] == mychar),
                ...
                if any(
                    [
                        (cells[1][1] == mychar and cells[0][1] == mychar),
                ...
                if any(
                    [
                        (cells[1][1] == mychar and cells[1][2] == mychar),
                ...
                if any(
                    [
                        (cells[0][0] == mychar and cells[2][2] == mychar),
...
def game(player1: str, player2: str, silent: bool = False) -> int:
    ...
    for p in players:
        if p not in ("human", "easy", "medium", "hard"):
            raise SystemError(
                "player1, player2 must be: " + "'human', 'easy', 'medium','hard'"
            )
    ...
    for turns in range(9):  # play 9 legal moves, then the board is full
        ...
        while True:  # ask until input is acceptable
            if players[playerindex] == "human":  # human player
                ...
                command = choose_a_free_cell(
                    cells, player_char, players[playerindex], silent
                )
            if not silent:
                print(
                    "player{} ({}{}) plays: {}".format(
                        playerindex + 1, players[playerindex], suffix, command
                    )
                )
        ...
        if check_win(player_char, cells):  # only the active player is checked
            if not silent:
                print(
                    "Congratulation, player {} ({}{}) has won!".format(
