...
for pair in pairings:
    ...
    # print(winners)
    print(
        f"{pair[0]} vs {pair[1]}: wins:{winners[1]} losses:{winners[2]} draws:{[winners[3]]}"
    )
    if graphic:
        ...
        pie_chart.add(
            "wins: {} ({:.1f}%)".format(winners[1], winners[1] / runs * 100), winners[1]
        )
        pie_chart.add(
            "losses: {} ({:.1f}%)".format(winners[2], winners[2] / runs * 100),
            winners[2],
        )
        pie_chart.add(
            "draws: {} ({:.1f}%)".format(winners[3], winners[3] / runs * 100),
            winners[3],
        )
