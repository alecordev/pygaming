        ...
        for x, value in enumerate(line):
            ...
            for (dx, dy) in (
                ...
                if Config.wrap_around:
                    ...
                    if (
                        (y + dy) < 0
                        or (y + dy) >= len(Config.array)
                        or (x + dx) < 0
                        or (x + dx) >= len(Config.array[0])
                    ):
                        continue
            ...
            if (
                Config.array[y][x]
                and counter >= Config.stay_alive_min
                and counter <= Config.stay_alive_max
            ):
                new[y][x] = True
            # dead cell becomes alive when exactly 3 neighbors
            elif (
                not Config.array[y][x]
                and counter >= Config.reborn_min
                and counter <= Config.reborn_max
            ):
                new[y][x] = True
    return new
...
def create_layout():
    ...
    layout.append(
        [
            sg.Button(button_text="next"),
            sg.Text("turn:"),
            sg.Text(str(Config.turn), key="turn", size=(5, 0)),
        ]
    )
    layout.append(
        [
            sg.Text("reborn min:", size=(13, 0)),
            sg.Input(str(Config.reborn_min), key="reborn_min", size=(2, 0)),
            sg.Text("reborn max:", size=(13, 0)),
            sg.Input(str(Config.reborn_max), key="reborn_max", size=(2, 0)),
        ]
    )
    layout.append(
        [
            sg.Text("stay alive min:", size=(13, 0)),
            sg.Input(str(Config.stay_alive_min), key="stay_alive_min", size=(2, 0)),
            sg.Text("stay alive max:", size=(13, 0)),
            sg.Input(str(Config.stay_alive_max), key="stay_alive_max", size=(2, 0)),
        ]
    )
    layout.append(
        [
            sg.Checkbox(text="wrap-around", default=True, key="wrap_around"),
            sg.Button("reconfig"),
        ]
    )
