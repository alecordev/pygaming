...
class Star:
    ...
    def update(self, seconds):
        ...
        # Grow brighter
        self.brightness = min(
            self.brightness + Star.warp_factor * 200 * seconds, self.speed, 150
        )
...
class Viewer:
    ...
    def create_star(self):
        ...
        pos = pygame.math.Vector2(
            self.screenrect.centerx + dx * d, self.screenrect.centery + dy * d
        )
    ...
    def run(self):
        ...
        while running:
            ...
            Star.stars = [
                s for s in Star.stars if self.screenrect.collidepoint(s.end_pos)
            ]
            # ---- draw ----
            for s in Star.stars:
                s.draw(self.screen)
            # ---- gui ----
            text = "fps: {:.2f} stars: {} warp: {:.2f} Press ESC to quit".format(
                self.clock.get_fps(), len(Star.stars), Star.warp_factor
            )
