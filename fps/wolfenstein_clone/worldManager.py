...
class WorldManager(object):
    def __init__(self, worldMap, sprite_positions, x, y, dirx, diry, planex, planey):
        self.sprites = [
            load_image(
                pygame.image.load("pics/items/barrel.png").convert(),
                False,
                colorKey=(0, 0, 0),
            ),
            load_image(
                pygame.image.load("pics/items/pillar.png").convert(),
                False,
                colorKey=(0, 0, 0),
            ),
            load_image(
                pygame.image.load("pics/items/greenlight.png").convert(),
                False,
                colorKey=(0, 0, 0),
            ),
        ]
        ...
        self.images = [
            ...
            load_image(
                pygame.image.load("pics/walls/purplestone.png").convert(), False
            ),
    ...
    def draw(self, surface):
        ...
        if self.background is None:
            self.background = pygame.transform.scale(
                pygame.image.load("pics/background.png").convert(), (w, h)
            )
        ...
        for x in range(w):
            ...
            surface.blit(
                pygame.transform.scale(self.images[texNum][texX], (1, lineHeight)),
                (x, drawStart),
            )
        ...
        def sprite_compare(s1, s2):
            ...
            s1Dist = math.sqrt(
                (s1[0] - self.camera.x) ** 2 + (s1[1] - self.camera.y) ** 2
            )
            s2Dist = math.sqrt(
                (s2[0] - self.camera.x) ** 2 + (s2[1] - self.camera.y) ** 2
            )
            if s1Dist > s2Dist:
                return -1
            elif s1Dist == s2Dist:
        ...
        for sprite in sorted(self.sprite_positions):
            ...
            invDet = 1.0 / (
                self.camera.planex * self.camera.diry
                - self.camera.dirx * self.camera.planey
            )  # required for correct matrix multiplication

            transformX = invDet * (
                self.camera.diry * spriteX - self.camera.dirx * spriteY
            )
            transformY = invDet * (
            ...
            if spriteHeight < 1000:
                for stripe in range(int(drawStartX), int(drawEndX)):
                    texX = (
                        int(
                            256
                            * (stripe - (-spriteWidth / 2 + spritesurfaceX))
                            * texWidth
                            / spriteWidth
                        )
                        / 256
                    ...
                    if (
                        ...
                        and stripe > 0
                        and stripe < w
                        and transformY < zBuffer[stripe]
                    ):
                        surface.blit(
                            pygame.transform.scale(
                                self.sprites[sprite[2]][int(texX)], (1, spriteHeight)
                            ),
                            (stripe, drawStartY),
                        )
