...
class Item(pygame.sprite.Sprite):
    ...
    def __init__(
        self, name, quantity, coords, destinationGroup="default", imageName=None
    ):
    ...
    def update(self):
        if my.allItems.has(self):
            ...
            if not self.beingCarried:
                ...
                if self.reserved and self.coords not in [
                    self.reserved.coords,
                    self.reserved.destination,
                ]:
                    self.reserved = None
...
class Order:
    ...
    def __init__(
        self, itemName, prerequisites, building, constructionTicks, itemQuantity
    ):
    ...
    def update(self, building, dt):
        ...
        if self.constructionProgress >= 0:
            ...
            if (
                self.constructionProgress >= self.constructionTicks
