...
class Handler:
    ...
    def update(self, deltaTime):
        ...
        if pygame.locals.K_SPACE in my.input.unpressedKeys:
            ...
            self.pauseTextSurf, self.pauseTextRect = ui.genText(
                "PAUSED (press space to unpause)", (10, 10), my.WHITE, ui.MEGAFONT
            )
            self.pauseTextRect.center = (
                int(my.WINDOWWIDTH / 2),
        ...
        if my.paused:
            if self.pauseAlpha < 150:
                self.pauseAlpha += 600 * deltaTime
    ...
    def newGame(self):
        ...
        for i in range(6):
            ...
            human.destination = (
                random.randint(int(my.MAPXCELLS / 2) - 5, int(my.MAPXCELLS / 2) + 5),
