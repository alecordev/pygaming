...
class MoveCamera:
    def __init__(self):
        ...
        self.description = (
            "Use WASD keys or the mouse at the edge of the screen to pan the camera"
        )
    ...
    def getProgress(self):
        ...
        if my.camera.focus != self.startFocus:
            return 100
        return 0
...
class RecruitOccupation:
    ...
    def getProgress(self):
        for human in my.allHumans:
            if human.occupation == self.occupation:
                return 100

        if self.occupation == "jobless":
            return 0
        ...
        if tip in my.UItips:  # if a new tip was created
            ...
            tip.tooltip.rect.topright = (
                occAssRect.left + 28 * isSecondColumn - ui.GAP,
                occAssRect.top + yValue * 28 + ui.GAP * 2,
            )
