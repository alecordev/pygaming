...
my.gameRunning = (
    my.DEBUGMODE
)  # set to True to skip menus and use default worldgen settings
...
def run():
    ...
    while True:  # main game loop
        ...
        pygame.display.set_caption(
            ...
            + " " * 10
            + "FPS: "
            + str(int(my.FPSCLOCK.get_fps()))
        )


class MainMenu:
    def __init__(self):
        self.logoImg = pygame.image.load(
            "assets/aedificus title and dude.png"
        ).convert_alpha()
        self.logoRect = self.logoImg.get_rect()
        self.logoRect.midtop = (my.HALFWINDOWWIDTH, -773)

        self.versionSurf, self.versionRect = ui.genText(
            "Aedificus " + str(my.VERSIONNUMBER), (0, 0), my.WHITE, ui.BIGFONT
        )
        ...
        self.quitButton = ui.Button(
            " Quit ", 0, (my.HALFWINDOWWIDTH - 100, my.WINDOWHEIGHT), 1, 1
        )
    ...
    def update(self):
        ...
        if not self.animateOut:
            if self.playButton.rect.y > my.HALFWINDOWHEIGHT + 20:
                self.playButton.rect.y -= (
                    math.fabs(my.HALFWINDOWHEIGHT + 300 - self.playButton.rect.y) * 0.1
                )
            if self.quitButton.rect.y > my.HALFWINDOWHEIGHT + 100:
                self.quitButton.rect.y -= (
                    math.fabs(my.HALFWINDOWHEIGHT + 380 - self.quitButton.rect.y) * 0.1
                )
            ...
            if self.versionRect.bottom > my.WINDOWHEIGHT - ui.GAP:
                self.versionRect.bottom -= (
                    math.fabs(my.WINDOWHEIGHT - 5 - self.versionRect.bottom) * 0.1
                )
            ...
            if self.playButton.rect.y < my.WINDOWHEIGHT - 1:
                animateDone = False
                self.playButton.rect.y += (
                    my.WINDOWHEIGHT + 50 - self.playButton.rect.y
                ) * 0.1
            if self.quitButton.rect.y < my.WINDOWHEIGHT - 1:
                animateDone = False
                self.quitButton.rect.y += (
                    my.WINDOWHEIGHT + 50 - self.quitButton.rect.y
                ) * 0.1
                self.creditsButton.rect.y = self.quitButton.rect.y
...
class EmbarkMenu:
    ...
    def __init__(self):
        self.sliders = []
        self.sliderData = [
            {"label": "Number of mountains", "valRange": (5, 25), "default": 10},
        ...
        for data in self.sliderData:
            self.sliders.append(
                ui.Slider(
                    (
                        int(my.WINDOWWIDTH / 2 - ui.Slider.size[0] / 2),
                        200 + i * ui.Slider.size[1] + i * ui.GAP,
                    ),
                    data["valRange"],
                    data["label"],
                    data["default"],
        ...
        self.backButton = ui.Button(
            " Back ", 0, (my.HALFWINDOWWIDTH - 100, my.WINDOWHEIGHT - 100), 1, 1
        )
        self.embarkButton = ui.Button(
            "Embark", 0, (my.HALFWINDOWWIDTH + 20, my.WINDOWHEIGHT - 100), 1, 1
        )

        self.logoImg = pygame.image.load(
            "assets/aedificus title smaller.png"
        ).convert_alpha()
        self.logoRect = self.logoImg.get_rect()
    ...
    def update(self):
        ...
        if self.embarkButton.isClicked:
            ...
            self.loadingSurf, self.loadingRect = ui.genText(
                "GENERATING WORLD", (0, 0), my.WHITE, ui.MEGAFONT
            )
        ...
        if self.animateOut:  # animate out
            ...
            if self.backButton.rect.y < my.WINDOWHEIGHT - 1:
                animateDone = False
                self.backButton.rect.y += (
                    my.WINDOWHEIGHT + 50 - self.backButton.rect.y
                ) * 0.1
            ...
            if self.animateOut == "play":
                my.screen.fill(
                    my.DARKBLUE,
                    pygame.Rect(
                        self.loadingRect.x - 20,
                        self.loadingRect.y - 20,
                        self.loadingRect.width + 40,
                        self.loadingRect.height + 40,
                    ),
                )
            ...
            if self.backButton.rect.y > my.HALFWINDOWWIDTH - 100:
                self.backButton.rect.y -= (
                    math.fabs(my.WINDOWHEIGHT * 0.75 - self.backButton.rect.y) * 0.1
                )
                self.embarkButton.rect.y = self.backButton.rect.y
...
class CreditsMenu:
    def __init__(self):
        ...
        self.thanksSurf, self.thanksRect = ui.genText(
            "With thanks to Mekire", (0, 0), my.WHITE, ui.BIGFONT
        )
        ...
        self.soundsButton = ui.Button(
            "Sounds courtesy of all these lovely people", 0, (0, 0), 1, 1
        )
    ...
    def update(self):
        ...
        if self.soundsButton.isClicked:
            webbrowser.open(
                "http://www.freesound.org/people/jellyberg/bookmarks/category/27033/", 2
            )
        ...
        if not self.animateOut:
            ...
            if self.backButton.rect.y > my.HALFWINDOWHEIGHT + 100:
                self.backButton.rect.y -= (
                    math.fabs(my.HALFWINDOWHEIGHT + 100 - self.backButton.rect.y) * 0.1
                )
            if self.thanksRect.y > my.HALFWINDOWHEIGHT + 60:
                self.thanksRect.y -= (
                    math.fabs(my.HALFWINDOWHEIGHT + 50 - self.thanksRect.y) * 0.1
                )
            if self.soundsButton.rect.y > my.HALFWINDOWHEIGHT + 10:
                self.soundsButton.rect.y -= (
                    math.fabs(my.HALFWINDOWHEIGHT + 10 - self.soundsButton.rect.y) * 0.1
                )
            ...
            if self.backButton.rect.y < my.WINDOWHEIGHT - 1:
                ...
                self.soundsButton.rect.y += (
                    my.WINDOWHEIGHT + 50 - self.soundsButton.rect.y
                ) * 0.1
